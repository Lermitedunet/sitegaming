...
(function initInputModality(){
  ...
  const set = (v) => { root.dataset.input = v; };
...
function getTeamData() {
    try {
        ...
        if (raw) {
            ...
            } else if (stored && typeof stored === 'object' && stored.members && Array.isArray(stored.members)) {
                // Nouveau format
    ...
    return normalizeTeamData(BASE_TEAM);
}
...
function migrateTeamFromArray(oldArray) {
    const members = oldArray.map((member, index) => {
        ...
        if (roleText.includes('directeur') || roleText.includes('director')) {
            role = 'directeur';
        } else if (roleText.includes('créateur') || roleText.includes('createur') || roleText.includes('créateur')) {
            role = 'createur';
        }
        ...
        let avatarMediaId = '';
        if (avatarUrl && avatarUrl.trim() && (avatarUrl.startsWith('http://') || avatarUrl.startsWith('https://'))) {
            avatarType = 'url';
        } else {
            avatarType = 'initials';
        }
...
function normalizeTeamData(teamData) {
    if (!teamData || typeof teamData !== 'object') {
        return { aboutTitle: "Équipe", aboutText: "Les admins et rédacteurs du site", members: [] };
    }
    
    return {
        aboutTitle: (teamData.aboutTitle || "Équipe").trim(),
        aboutText: (teamData.aboutText || "Les admins et rédacteurs du site").trim(),
        members: Array.isArray(teamData.members) ? teamData.members.map(m => normalizeTeamMember(m)).filter(m => m !== null) : []
    };
}
...
function normalizeTeamMember(member) {
    ...
    const role = member.role === 'directeur' || member.role === 'createur' || member.role === 'redacteur' 
        ? member.role 
        : 'redacteur';
    
    const order = typeof member.order === 'number' && member.order >= 1 ? member.order : 999;
    
    // ADDED: Support avatarType "image" pour avatars avec recadrage
    const avatarType = member.avatarType === 'media' || member.avatarType === 'url' || member.avatarType === 'image' || member.avatarType === 'initials'
        ? member.avatarType
        : 'initials';
    ...
    const isActive = member.isActive !== undefined ? Boolean(member.isActive) : true;
    
    // ADDED: avatarText (initiales par défaut)
    const avatarText = (member.avatarText || getInitials(name)).trim().substring(0, 3).toUpperCase();
    ...
    return {
        ...
        avatarCrop: (member.avatarCrop && typeof member.avatarCrop === 'object') ? {
            x: typeof member.avatarCrop.x === 'number' ? member.avatarCrop.x : 0,
            y: typeof member.avatarCrop.y === 'number' ? member.avatarCrop.y : 0,
            scale: typeof member.avatarCrop.scale === 'number' && member.avatarCrop.scale >= 1 ? member.avatarCrop.scale : 1
        } : { x: 0, y: 0, scale: 1 }, // ADDED: Crop pour avatars image
        initials: (member.initials || avatarText).trim().substring(0, 3).toUpperCase(),
        ...
        links: {
            ...
            site: ((member.links || {}).site || (member.links || {}).website || '').trim()
        }
    };
}
...
function saveTeamData(teamData) {
    try {
        ...
        localStorage.setItem(window.LMD.storageKeys.team, JSON.stringify(normalized));
        console.log('[team] saved team data', normalized.members?.length || 0, 'members');
    } catch (e) {
...
function getArticlesByAuthorName(authorName) {
    ...
    return articles.filter(article => {
        ...
        const articleAuthor = (toText(article.authorName || article.author || '')).trim();
...
function ensureUniqueMemberSlug(members, desiredSlug, currentId) {
    try {
        ...
        while (members.some(m => {
            const normalized = normalizeTeamMember(m);
            return normalized && normalized.slug === candidate && normalized.id !== currentId;
        })) {
...
function getTeamMembers() {
    ...
    const members = (teamData && Array.isArray(teamData.members)) ? teamData.members : [];
...
function normalizeAuthorId(value) {
    ...
    try {
        ...
        if (trimmedLower === 'clement' || trimmedLower === 'yanis' || trimmedLower === 'leo') {
            return trimmedLower;
        }
        
        // Mapping spécifique selon les noms
        const lower = trimmedLower;
        if (lower.includes('clément') || lower.includes('clement') || lower.includes('pingon clément') || lower.includes('pingon clement')) {
            return 'clement';
        }
        if (lower.includes('yanis') || lower.includes('pingon yanis')) {
            return 'yanis';
        }
        if (lower.includes('léo') || lower.includes('leo') || lower.includes('atake léo') || lower.includes('atake leo')) {
            return 'leo';
        }
...
function sanitizeTeamMember(member) {
    ...
    return {
        ...
        links: {
            ...
            website: ((member.links || member.socials || {}).website || (member.links || member.socials || {}).site || '').trim()
        }
    };
}
...
function getPromosData() {
    ...
    if (raw) {
        try {
            ...
            console.warn('Erreur lors de la lecture des promos depuis localStorage:', e);
        }
    }
    return [];
}
...
const GAMES_DB = {
    "neon-rift": {
        ...
        gameplay: "Plongez dans des combats rapides et intenses avec un arsenal d'armes futuristes. Maîtrisez les mouvements fluides, les sauts boostés et les capacités spéciales pour dominer vos adversaires dans des arènes multijoueur compétitives.",
        ...
        imageGradient: "blue",
        articles: [
            { title: "Guide des meilleurs FPS", href: "articles/guide-meilleurs-fps.html", tag: "Guide" },
            { title: "Nouveau jeu AAA sorti", href: "articles/nouveau-jeu-aaa.html", tag: "Actu" }
        ]
    },
    "mythfall": {
        title: "MYTHFALL",
        ...
        gameplay: "Explorez un vaste monde ouvert rempli de quêtes, de donjons et de secrets. Développez votre personnage avec un système de compétences profond, forgez des armes légendaires et affrontez des boss épiques dans des combats tactiques.",
        features: ["Solo", "Coop"],
        studio: "Mythic Games",
        release: "2026",
        duration: "60–80h",
    ...
    "iron-circuit": {
        ...
        gameplay: "Pilotez des véhicules de combat personnalisables dans des courses intenses. Utilisez des armes montées, des boosters et des capacités spéciales pour éliminer vos adversaires tout en restant en tête de la course.",
        features: ["Solo", "PvP"],
    ...
    "pocket-odyssey": {
        ...
        gameplay: "Explorez un monde ouvert rempli de secrets et de défis. Résolvez des puzzles environnementaux, interagissez avec des personnages mémorables et collectez des objets rares pour progresser dans votre quête épique.",
        features: ["Solo"],
...
function initGridPagination({
  ...
  if (!resetPage) {
    ...
    const urlPage = parseInt(urlParams.get('p') || urlParams.get('page') || '1');
...
function applyPaginationToContainer(containerId, items, renderItemFn, pageSize = 12) {
...
function renderTestsPage() {
    ...
    function renderTestItem(test) {
        const game = test.gameId ? getGamesData().find(g => g.id === test.gameId) : null;
        const gameName = game ? game.title : (test.gameTitle || 'Jeu inconnu');
        const scoreText = test.score ? `${test.score.toFixed(1)}/10` : 'N/A';
        const platforms = Array.isArray(test.platforms) && test.platforms.length > 0
            ? test.platforms.join(', ')
            : 'Toutes plateformes';

        // Image de couverture (comme les articles) avec badge de score superposé
        const imageHtml = test.coverUrl && test.coverUrl.trim()
        ...
        const dateText = test.publishedAt ? ` • ${new Date(test.publishedAt).toLocaleDateString('fr-FR')}` : '';

        // Générer le lien vers la page détail du test
        const testUrl = test.slug ? `test.html?slug=${encodeURIComponent(test.slug)}` : `test.html?id=${encodeURIComponent(test.id)}`;
...
function updateDebugPanel() {
    ...
    if (content) {
        ...
        if (Object.keys(seeded).length > 0) {
            html += '<div style="margin-bottom: 8px;"><strong>🧪 Recent Seed:</strong></div>';
        ...
        // Résultats d'audit
        if (auditResults) {
            html += '<div style="margin-bottom: 8px;"><strong>🔍 Audit Results:</strong></div>';
        ...
        // Logs récents (max 10)
        if (logs.length > 0) {
            html += '<div style="margin-bottom: 8px;"><strong>📝 Recent Logs:</strong></div>';
            logs.slice(-10).forEach(log => {
                const color = log.level === 'error' ? '#ef4444' : log.level === 'warn' ? '#fbbf24' : '#4ade80';
...
function setupErrorCapturing() {
    ...
    window.addEventListener('error', (event) => {
        debugLog('ERROR', `JS Error: ${event.message} at ${event.filename}:${event.lineno}`, 'error');
    });
    ...
    document.addEventListener('error', (event) => {
        if (event.target.tagName === 'IMG' && event.target.closest('.admin-list, .admin-item')) {
...
const TEST_STEPS = [
    {
        ...
        description: 'Ajoutez 20 jeux réels via l\'onglet "Jeux". Utilisez les templates pour faciliter la saisie.',
        targetCount: 20,
        contentType: 'games',
        templates: [
            {
                title: 'The Legend of Zelda: Tears of the Kingdom',
                description: 'Suite de Breath of the Wild avec de nouvelles mécaniques et un monde ouvert immense.',
                image: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063780/9989957eae3a6b545194c42fec207aad5b2cc1043c8e4a4a6a646e47544b7b0fc',
                platforms: ['Switch'],
            ...
            },
            {
                ...
                description: 'RPG tactique basé sur D&D avec un système de combat au tour par tour révolutionnaire.',
    ...
    },
    {
        ...
        description: 'Ajoutez 13 articles dans la catégorie "Actu" via l\'onglet "Articles".',
        targetCount: 13,
        ...
        templates: [
            {
                ...
                excerpt: 'Sony dévoile ses prochains hits avec Spider-Man 3, God of War Ragnarök Valhalla et bien d\'autres surprises.',
                cover: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202309/2702/52f0f5c0343b72b4bd6f5c6c3a2bb5c8a2e6b8c6.png',
                category: 'actu'
            }
        ]
    },
    {
        ...
        description: 'Ajoutez 13 guides via l\'onglet "Articles" avec catégorie "Guide".',
        targetCount: 13,
        contentType: 'articles',
        category: 'guide',
        templates: [
            {
                ...
                excerpt: 'Maîtrisez toutes les mécaniques et explorez chaque recoin du royaume d\'Hyrule.',
                cover: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063780/9989957eae3a6b545194c42fec207aad5b2cc1043c8e4a4a6a646e47544b7b0fc',
                category: 'guide'
            }
        ]
    },
    {
        ...
        description: 'Ajoutez 13 tests via l\'onglet "Tests". Liez chaque test à un jeu existant.',
        targetCount: 13,
        contentType: 'tests',
        templates: [
            {
                title: 'Test complet Zelda : Tears of the Kingdom',
                excerpt: 'Une suite exceptionnelle qui élève encore plus haut la barre de l\'open-world.',
                content: 'Zelda : Tears of the Kingdom est tout simplement exceptionnel...',
                coverUrl: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063780/9989957eae3a6b545194c42fec207aad5b2cc1043c8e4a4a6a646e47544b7b0fc',
                score: 9.5,
                criteria: [
        ...
        templates: [
            {
                ...
                excerpt: 'Profitez de la meilleure réduction actuelle sur le jeu de l\'année 2022.',
                coverUrl: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202207/1210/4xJ8XB3bi888QTLZYdl7Oi0.png',
                externalUrl: 'https://store.steampowered.com/app/2344520/God_of_War_Ragnark/',
                merchant: 'Steam',
                price: 39.99,
                oldPrice: 99.99,
                discountPercent: 60
    ...
    checklistEl.innerHTML = TEST_STEPS.map((step, index) => {
        ...
        return `
            <div class="test-step ${isCurrent ? 'current' : ''} ${isCompleted ? 'completed' : ''}" data-step="${index}">
                ...
                ${isCurrent ? `<div class="test-step-actions">
...
function showTestTemplates(stepIndex) {
    ...
    const templatesHtml = step.templates.map((template, index) => `
...
function copyTemplateToForm(contentType, template) {
    try {
        const data = typeof template === 'string' ? JSON.parse(template.replace(/&quot;/g, '"')) : template;
        ...
        logTestAction(`Template copié pour ${contentType}: ${data.title || data.name}`);
    } catch (error) {
...
function copyGameTemplate(template) {
    ...
    const addBtn = document.querySelector('#btn-add-game, [data-admin-tab="games"] + .admin-content-section .admin-actions .btn');
    if (addBtn) {
        addBtn.click();
        // Attendre que le modal s'ouvre, puis remplir
        setTimeout(() => {
            document.getElementById('game-title').value = template.title || '';
            document.getElementById('game-description').value = template.description || '';
...
function copyTestTemplate(template) {
    ...
    if (addBtn) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('test-publishedAt').value = template.publishedAt || '';
            // TODO: gérer critères
        }, 100);
    }
}
...
function copyBonPlanTemplate(template) {
    ...
    if (addBtn) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('bon-plan-excerpt').value = template.excerpt || '';
            document.getElementById('bon-plan-cover').value = template.coverUrl || '';
            document.getElementById('bon-plan-external-url').value = template.externalUrl || '';
            document.getElementById('bon-plan-merchant').value = template.merchant || '';
            document.getElementById('bon-plan-price').value = template.price || '';
            document.getElementById('bon-plan-old-price').value = template.oldPrice || '';
            document.getElementById('bon-plan-discount').value = template.discountPercent || '';
        }, 100);
    }
}
...
function getCurrentCount(step) {
    try {
        switch (step.contentType) {
            ...
            case 'articles':
                return getArticlesData().filter(a => a.category === step.category).length;
            case 'tests':
...
function runStepVerification(stepIndex) {
    ...
    if (isCompleted) {
        logTestAction(`✅ Étape terminée: ${currentCount}/${step.targetCount} items créés`, 'success');
        if (additionalChecks.length === 0) {
            logTestAction('✅ Toutes les vérifications passées', 'success');
        }
    } else {
        logTestAction(`⏳ Étape en cours: ${currentCount}/${step.targetCount} items créés`, 'info');
    }

    additionalChecks.forEach(check => logTestAction(check, check.includes('❌') ? 'error' : 'info'));
...
function verifyTestsData() {
    ...
    tests.forEach(test => {
        if (!test.gameId) {
            ...
            if (!game) {
                issues.push(`❌ Test "${test.title}": jeu référencé inexistant (${test.gameId})`);
            } else if (test.platforms && test.platforms.length > 0) {
                // Vérifier cohérence plateformes
                const testPlatforms = test.platforms;
                const gamePlatforms = game.platforms || [];
                const hasCommonPlatform = testPlatforms.some(p => gamePlatforms.includes(p));
                if (!hasCommonPlatform && gamePlatforms.length > 0) {
                    issues.push(`⚠️ Test "${test.title}": plateformes incohérentes avec le jeu`);
                }
            }
        }
    });
...
function updateTestNavigation() {
    ...
    if (nextBtn) {
        ...
        nextBtn.textContent = currentTestStep === TEST_STEPS.length - 1 ? 'Terminer' : 'Étape suivante';
    }
}
...
function goToNextTestStep() {
    if (currentTestStep < TEST_STEPS.length - 1) {
        ...
        // Test terminé
        logTestAction('🎉 Test admin terminé ! Toutes les étapes validées.', 'success');
        toggleAdminTestMode();
    }
}
...
function getStableImage(gameId, fallbackUrl) {
    ...
    return fallbackUrl || 'https://cdn.cloudflare.steamstatic.com/steam/apps/271590/header.jpg'; // GTA V fallback
}
...
const DEMO_CONTENT = {
    games: [
        {
            id: 'zelda-totk',
            title: 'The Legend of Zelda: Tears of the Kingdom',
            description: 'Suite exceptionnelle de Breath of the Wild avec de nouvelles mécaniques de construction et d\'exploration verticale révolutionnaires.',
            image: 'https://www.nintendo.com/content/dam/noa/en_US/games/switch/t/the-legend-of-zelda-tears-of-the-kingdom-switch/the-legend-of-zelda-tears-of-the-kingdom-switch-hero.jpg',
            platforms: ['Switch'],
            genres: ['Action', 'Aventure'],
            rating: 4.8,
            demo: true
        },
        {
            id: 'baldurs-gate-3',
            title: 'Baldur\'s Gate 3',
            description: 'RPG tactique révolutionnaire basé sur les règles D&D, offrant une liberté d\'action et de narration sans précédent.',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1086940/capsule_616x353.jpg',
            platforms: ['PC', 'PS5', 'Xbox Series X'],
            genres: ['RPG'],
            rating: 4.9,
        ...
        },
        {
            id: 'elden-ring',
            title: 'Elden Ring',
            description: 'Action-RPG ouvert créé par FromSoftware et George R.R. Martin, alliant exploration, combat difficile et lore riche.',
        ...
        },
        {
            ...
            description: 'RPG futuriste dans un monde ouvert dystopique, avec des choix narratifs impactants et un univers cyberpunk riche.',
        ...
        },
        {
            ...
            description: 'Suite épique de la série God of War, concluant l\'histoire de Kratos et Atreus dans les royaumes nordiques.',
        ...
        },
        {
            ...
            description: 'Nouvelle aventure de Spider-Man avec Peter Parker et Miles Morales, dans une New York plus grande et plus vivante.',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2652250/header.jpg',
            platforms: ['PS5'],
            genres: ['Action', 'Aventure'],
            rating: 4.6,
            demo: true
        },
        {
            id: 'helldivers-2',
            title: 'Helldivers 2',
            description: 'Shoot\'em up coopératif chaotique où les joueurs combattent des hordes d\'aliens dans une guerre galactique.',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/553850/capsule_616x353.jpg',
            platforms: ['PC', 'PS5'],
        ...
        },
        {
            id: 'alan-wake-2',
            title: 'Alan Wake 2',
            description: 'Suite du survival horror narratif, mêlant enquête, action et éléments de réalité altérée.',
            image: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1310/42b1baff1b6e8a0b4fb4f0f2d2c8b0b9e7f3e8f8.png',
            platforms: ['PC', 'PS5', 'Xbox Series X'],
        ...
        },
        {
            ...
            description: 'Épopée western magistrale avec un monde ouvert immense et une histoire captivante dans l\'Amérique du début du XXème siècle.',
            image: 'https://cdn.akamai.steamstatic.com/cdn/EP1003/CUSA03041_00/OFICSTOREART0.png',
            platforms: ['PC', 'PS5', 'Xbox Series X'],
            genres: ['Action', 'Aventure'],
            rating: 4.8,
            demo: true
        },
        {
            ...
            description: 'RPG épique avec un monde ouvert fantastique, des choix moraux impactants et une quête principale mémorable.',
        ...
        },
        {
            id: 'hades',
            title: 'Hades',
            description: 'Roguelike d\'action avec une histoire captivante, des mécaniques de combat excellentes et une direction artistique remarquable.',
        ...
        },
        {
            id: 'hollow-knight',
            title: 'Hollow Knight',
            description: 'Metroidvania 2D atmosphérique avec un univers riche, des combats exigeants et une exploration gratifiante.',
        ...
        },
        {
            id: 'diablo-iv',
            title: 'Diablo IV',
            description: 'Action-RPG traditionnel de Blizzard, revenant aux racines de la série avec un monde ouvert sombre et addictif.',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2344520/capsule_616x353.jpg',
            platforms: ['PC', 'PS5', 'Xbox Series X'],
            genres: ['Action', 'RPG'],
            rating: 3.8,
            demo: true
        },
        {
            ...
            description: 'RPG d\'action dans l\'univers Harry Potter, permettant d\'explorer Poudlard et ses environs magiques.',
        ...
        },
        {
            id: 'starfield',
            title: 'Starfield',
            description: 'RPG spatial ambitieux développé par Bethesda, explorant la galaxie et les colonies humaines.',
        ...
        },
        {
            ...
            description: 'Monstre sacré du jeu ouvert, avec trois protagonistes, une histoire démente et un monde de jeu immense.',
        ...
        },
        {
            id: 'among-us',
            title: 'Among Us',
            description: 'Jeu multijoueur de déduction sociale où les joueurs doivent identifier les imposteurs parmi l\'équipage.',
        ...
        },
        {
            id: 'fortnite',
            title: 'Fortnite',
            description: 'Battle Royale avec construction, événements spéciaux et mode créatif, devenu un phénomène culturel.',
            image: 'https://cdn2.unrealengine.com/fortnite-marketing-site/en-US/social-16x9.jpg',
        ...
        },
        {
            id: 'rocket-league',
            title: 'Rocket League',
            description: 'Sport mécanique mélangeant football et voitures, avec des mécaniques de jeu accessibles et addictives.',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/252950/capsule_616x353.jpg',
        ...
        },
        {
            ...
            description: 'Simulation agricole relaxante avec farming, relations sociales et exploration dans un monde pixel art.',
        ...
        actu: [
            {
                ...
                contentBlocks: [
                    { type: 'text', text: 'Le PlayStation Showcase 2024 a tenu toutes ses promesses avec des annonces majeures pour les fans de la PlayStation.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Microsoft impressionne avec de nouveaux titres Starfield, Forza Motorsport et des exclusivités Game Pass.',
                cover: 'https://news.xbox.com/en-us/wp-content/uploads/sites/2/2024/06/XboxGamesShowcase2024_KeyArt_1920x1080.jpg',
                category: 'actu',
                publishedAt: '2024-01-14T14:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Microsoft a présenté sa vision gaming pour les prochaines années avec des exclusivités impressionnantes.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Nintendo présente de nouveaux jeux pour Switch et tease la future console portable.',
                cover: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/en_US/articles/2024/nintendo-direct-6-18-2024/featured',
                category: 'actu',
                publishedAt: '2024-01-13T09:15:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Le Nintendo Direct de juin 2024 a mis l\'accent sur l\'innovation et les nouvelles expériences.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Valve améliore sa console portable avec un écran OLED et une meilleure autonomie.',
                cover: 'https://cdn.cloudflare.steamstatic.com/store/home/store_home_share.jpg',
                category: 'actu',
                publishedAt: '2024-01-12T16:45:00Z',
                contentBlocks: [
                    { type: 'text', text: 'La nouvelle Steam Deck OLED apporte des améliorations significatives à l\'expérience portable.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Le RPG de Larian Studios dépasse les 20 millions d\'exemplaires vendus.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1086940/capsule_616x353.jpg',
                category: 'actu',
                publishedAt: '2024-01-11T11:20:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Baldur\'s Gate 3 continue de cartonner et bat de nouveaux records de ventes.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'FromSoftware confirme l\'extension majeure pour le jeu acclamé.',
                cover: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202107/1612/Y5RHNmzAtc6sRYwZlZWkgOWJ.png',
                category: 'actu',
                publishedAt: '2024-01-10T13:10:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Shadow of the Erdtree s\'annonce comme une extension majeure pour Elden Ring.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'L\'extension de CD Projekt RED reçoit des critiques positives malgré les bugs initiaux.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1091500/capsule_616x353.jpg',
                category: 'actu',
                publishedAt: '2024-01-09T15:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Phantom Liberty apporte du contenu frais et de qualité à Cyberpunk 2077.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Le jeu Harry Potter dépasse les 25 millions de copies vendues.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/990080/capsule_616x353.jpg',
                category: 'actu',
                publishedAt: '2024-01-08T12:00:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Hogwarts Legacy confirme son statut de succès commercial majeur.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Les jeux vidéo génèrent plus de revenus que l\'industrie cinématographique.',
                cover: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=1200',
                category: 'actu',
                publishedAt: '2024-01-07T10:45:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Le gaming continue sa croissance fulgurante et dépasse désormais le cinéma.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Nintendo confirme enfin le développement du jeu tant attendu.',
                cover: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063780/9989957eae3a6b545194c42fec207aad5b2cc1043c8e4a4a6a646e47544b7b0fc',
                category: 'actu',
                publishedAt: '2024-01-06T14:20:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Metroid Prime 4 est enfin officiel et suscite beaucoup d\'enthousiasme.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'La plateforme bat ses records avec plus de 30 millions d\'utilisateurs simultanés.',
                cover: 'https://cdn.cloudflare.steamstatic.com/store/home/store_home_share.jpg',
                category: 'actu',
                publishedAt: '2024-01-05T09:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Steam continue de battre ses records de popularité.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Fifa devient EA Sports FC avec une nouvelle approche marketing.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2669320/capsule_616x353.jpg',
                category: 'actu',
                publishedAt: '2024-01-04T16:15:00Z',
                contentBlocks: [
                    { type: 'text', text: 'EA Sports FC 25 marque la fin de l\'ère FIFA pour Electronic Arts.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'L\'éditeur français annonce l\'ouverture de studios en Suède et en Espagne.',
                cover: 'https://staticctf.ubisoft.com/J3yJr34U2pZ2Ieem48Dwy69uqbjxBSd33xIvvzf2K9Q/prismic/field/image/Z2V0SW5Ub3VjaFdpdGhHYW1pbmd8ZWFhZWJiNjMtODM3ZC00NzY4LWI2ZGYtNmM5MmM3ZmM2MzM3fGVhLWFzc2V0cy1pbWFnZS1oZWFkZXI=',
                category: 'actu',
                publishedAt: '2024-01-03T11:45:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Ubisoft continue d\'investir dans de nouveaux studios européens.', blockId: 'block-1' }
                ],
                demo: true
            }
        ],
        ...
        guides: [
            {
                ...
                contentBlocks: [
                    { type: 'text', text: 'Guide complet pour maîtriser Zelda : Tears of the Kingdom.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Les meilleures combinaisons de classes et de sorts pour dominer les combats.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1086940/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-14T10:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des builds optimaux pour Baldur\'s Gate 3.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Découvrez tous les boss optionnels et les zones secrètes du jeu.',
                cover: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202107/1612/Y5RHNmzAtc6sRYwZlZWkgOWJ.png',
                category: 'guide',
                publishedAt: '2024-01-13T12:15:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des secrets d\'Elden Ring.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Maximisez votre expérience avec ces conseils pour les combats et l\'exploration.',
                cover: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202207/1210/4xJ8XB3bi888QTLZYdl7Oi0.png',
                category: 'guide',
                publishedAt: '2024-01-12T14:45:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Astuces essentielles pour God of War Ragnarök.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1219/1c2d8c1c87ab30eca44e72dc8741cd5d24ea85a8cfdb2f95.png',
                category: 'guide',
                publishedAt: '2024-01-11T09:20:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des routes optimisées pour Spider-Man 2.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2050650/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-10T16:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide de survie pour Resident Evil 4 Remake.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Visitez tous les districts et complétez votre carte de Night City.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1091500/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-09T11:10:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide d\'exploration pour Cyberpunk 2077.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Personnalisez votre demeure avec tous les secrets disponibles.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/990080/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-08T13:25:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des maisons magiques dans Hogwarts Legacy.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                excerpt: 'Maîtrisez les mécaniques de vol et d\'exploration de l\'espace.',
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1716740/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-07T15:40:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide de navigation spatiale pour Starfield.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1222670/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-06T10:55:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide de création de personnages pour The Sims 4.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/221100/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-05T12:30:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des constructions avancées Minecraft.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/945360/capsule_616x353.jpg',
                category: 'guide',
                publishedAt: '2024-01-04T14:15:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide des stratégies pour Among Us.', blockId: 'block-1' }
                ],
                demo: true
            },
            {
                ...
                cover: 'https://cdn2.unrealengine.com/fortnite-marketing-site/en-US/social-16x9.jpg',
                category: 'guide',
                publishedAt: '2024-01-03T16:50:00Z',
                contentBlocks: [
                    { type: 'text', text: 'Guide avancé pour Fortnite.', blockId: 'block-1' }
                ],
                demo: true
            }
        ]
    ...
    tests: [
        {
            ...
            title: 'Test complet Zelda : Tears of the Kingdom',
            excerpt: 'Une suite exceptionnelle qui élève encore plus haut la barre de l\'open-world.',
            content: 'Zelda : Tears of the Kingdom est tout simplement exceptionnel. Nintendo a réussi à innover tout en gardant l\'essence de la série. Les mécaniques de construction et de fusion sont révolutionnaires.',
            coverUrl: 'https://assets.nintendo.com/image/upload/c_fill,w_1200/q_auto:best/f_auto/dpr_2.0/ncom/software/switch/70010000063780/9989957eae3a6b545194c42fec207aad5b2cc1043c8e4a4a6a646e47544b7b0fc',
            gameId: 'zelda-totk',
            score: 9.5,
            criteria: [
        ...
        },
        {
            ...
            content: 'Baldur\'s Gate 3 est le jeu que tout amateur de RPG attendait. Larian Studios a créé un monde vivant avec des mécaniques de combat tactiques exceptionnelles.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1086940/capsule_616x353.jpg',
        ...
        },
        {
            ...
            excerpt: 'Difficile mais juste, FromSoftware livre une expérience inoubliable.',
            content: 'Elden Ring représente l\'apogée du travail de FromSoftware. La difficulté est légendaire mais toujours juste et gratifiante.',
            coverUrl: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202107/1612/Y5RHNmzAtc6sRYwZlZWkgOWJ.png',
            gameId: 'elden-ring',
            score: 9.3,
            criteria: [
        ...
        },
        {
            ...
            content: 'God of War Ragnarök achève magnifiquement la nouvelle trilogie. L\'histoire est touchante et les combats épiques.',
            coverUrl: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202207/1210/4xJ8XB3bi888QTLZYdl7Oi0.png',
        ...
        },
        {
            ...
            excerpt: 'Marvel\'s Spider-Man 2 confirme l\'excellence de la série sur PS5.',
            content: 'Spider-Man 2 élève encore la barre avec Miles Morales et ses nouvelles capacités électrifiées.',
            coverUrl: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1219/1c2d8c1c87ab30eca44e72dc8741cd5d24ea85a8cfdb2f95.png',
            gameId: 'spider-man-2',
            score: 8.9,
            criteria: [
                { label: 'Gameplay', score: 9.2 },
        ...
        },
        {
            ...
            excerpt: 'Un survival horror moderne qui respecte ses origines tout en innovant.',
            content: 'Capcom a parfaitement réussi son remake avec des graphismes modernes et des mécaniques améliorées.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/2050650/capsule_616x353.jpg',
            gameId: 'helldivers-2',
            score: 9.0,
            criteria: [
        ...
        },
        {
            ...
            content: 'Phantom Liberty transforme Cyberpunk 2077 en l\'expérience qu\'il aurait dû être dès le départ.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1091500/capsule_616x353.jpg',
        ...
        },
        {
            ...
            content: 'Avalanche Software livre une adaptation réussie de l\'univers Harry Potter avec un monde ouvert magique.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/990080/capsule_616x353.jpg',
        ...
        },
        {
            ...
            content: 'Starfield représente un énorme pas en avant pour Bethesda mais souffre de quelques lacunes.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1716740/capsule_616x353.jpg',
            gameId: 'starfield',
            score: 7.5,
            criteria: [
        ...
        },
        {
            ...
            excerpt: 'Une extension agricole qui apporte fraîcheur à la simulation de vie.',
            content: 'Cottage Living ramène The Sims 4 à ses racines agricoles avec de nombreuses nouvelles activités.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1222670/capsule_616x353.jpg',
        ...
        },
        {
            ...
            content: 'Vingt ans après sa sortie, Minecraft reste une référence incontournable du gaming.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/221100/capsule_616x353.jpg',
            gameId: 'minecraft',
            score: 9.8,
            criteria: [
        ...
        },
        {
            ...
            content: 'Among Us a révolutionné le gaming mobile et multijoueur avec ses mécaniques de déduction sociale.',
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/945360/capsule_616x353.jpg',
            gameId: 'among-us',
            score: 8.5,
            criteria: [
        ...
        },
        {
            ...
            content: 'Fortnite a transcendé le simple battle royale pour devenir un phénomène culturel à part entière.',
            coverUrl: 'https://cdn2.unrealengine.com/fortnite-marketing-site/en-US/social-16x9.jpg',
            gameId: 'fortnite',
            score: 8.0,
            criteria: [
    ...
    bonsPlans: [
        ...
        },
        {
            ...
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/221100/capsule_616x353.jpg',
            externalUrl: 'https://www.minecraft.net/fr-fr/store/minecraft-java-edition',
            merchant: 'Mojang',
            price: 26.95,
            oldPrice: 29.99,
            discountPercent: 10,
        ...
        },
        {
            ...
            coverUrl: 'https://cdn.cloudflare.steamstatic.com/steam/apps/252950/capsule_616x353.jpg',
    ...
    promos: [
        {
            ...
            image: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202207/1210/4xJ8XB3bi888QTLZYdl7Oi0.png',
        ...
        },
        {
            id: 'promo-elden-ring',
            title: 'Elden Ring -28%',
            image: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202107/1612/Y5RHNmzAtc6sRYwZlZWkgOWJ.png',
        ...
        },
        {
            ...
            image: 'https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1219/1c2d8c1c87ab30eca44e72dc8741cd5d24ea85a8cfdb2f95.png',
        ...
        },
        {
            id: 'promo-minecraft',
            title: 'Minecraft -10%',
            image: 'https://cdn.cloudflare.steamstatic.com/steam/apps/221100/capsule_616x353.jpg',
    ...
    partners: [
        ...
        },
        {
            id: 'partner-ubisoft',
            name: 'Ubisoft',
            description: 'Éditeur français de licences prestigieuses comme Assassin\'s Creed',
        ...
        },
        {
            id: 'partner-rockstar',
            name: 'Rockstar Games',
            description: 'Développeur de GTA, Red Dead Redemption et autres blockbusters',
...
function applyDemoInjection() {
    ...
    try {
        ...
        debugLog('DEMO', `Demo content injection completed: ${JSON.stringify(counts)}`);
        updateAdminSiteCounters();
...
function injectDemoGames() {
    ...
    const gamesToAdd = DEMO_CONTENT.games.filter(game => !existingIds.has(game.id));
    ...
    debugLog('DEMO', `Games: Added ${gamesToAdd.length} games (total: ${allGames.length})`);
}
...
function injectDemoArticles() {
    ...
    // Articles actu
    const actuToAdd = DEMO_CONTENT.articles.actu.filter(article => !existingIds.has(article.id));
    totalAdded += actuToAdd.length;

    // Guides
    const guidesToAdd = DEMO_CONTENT.articles.guides.filter(article => !existingIds.has(article.id));
    ...
    debugLog('DEMO', `Articles: Added ${totalAdded} articles (${actuToAdd.length} actu + ${guidesToAdd.length} guides, total: ${allArticles.length})`);
}
...
function injectDemoTests() {
    ...
    const testsToAdd = DEMO_CONTENT.tests.filter(test => !existingIds.has(test.id));
    ...
    debugLog('DEMO', `Tests: Added ${testsToAdd.length} tests (total: ${allTests.length})`);
}
...
function injectDemoBonsPlans() {
    ...
    const bonsPlansToAdd = DEMO_CONTENT.bonsPlans.filter(bp => !existingIds.has(bp.id));
    ...
    debugLog('DEMO', `Bons Plans: Added ${bonsPlansToAdd.length} deals (total: ${allBonsPlans.length})`);
}
...
function injectDemoPromos() {
    ...
    const promosToAdd = DEMO_CONTENT.promos.filter(promo => !existingIds.has(promo.id));
    ...
    debugLog('DEMO', `Promos: Added ${promosToAdd.length} promos (total: ${allPromos.length})`);
}
...
function injectDemoPartners() {
    ...
    const partnersToAdd = DEMO_CONTENT.partners.filter(partner => !existingIds.has(partner.id));
    ...
    debugLog('DEMO', `Partners: Added ${partnersToAdd.length} partners (total: ${allPartners.length})`);
}
...
function resetDemoContent() {
    ...
    try {
        ...
        debugLog('DEMO', `Demo content reset completed: ${JSON.stringify(resetCounts)}`);
        updateAdminSiteCounters();
...
function resetDemoGames() {
    ...
    if (filtered.length !== games.length) {
        ...
        debugLog('DEMO', `Games: Removed ${games.length - filtered.length} demo games`);
    }
}
...
function resetDemoArticles() {
    ...
    if (filtered.length !== articles.length) {
        ...
        debugLog('DEMO', `Articles: Removed ${articles.length - filtered.length} demo articles`);
    }
}
...
function resetDemoTests() {
    ...
    if (filtered.length !== tests.length) {
        ...
        debugLog('DEMO', `Tests: Removed ${tests.length - filtered.length} demo tests`);
    }
}
...
function resetDemoBonsPlans() {
    ...
    if (filtered.length !== bonsPlans.length) {
        ...
        debugLog('DEMO', `Bons Plans: Removed ${bonsPlans.length - filtered.length} demo deals`);
    }
}
...
function resetDemoPromos() {
    ...
    if (filtered.length !== promos.length) {
        ...
        debugLog('DEMO', `Promos: Removed ${promos.length - filtered.length} demo promos`);
    }
}
...
function resetDemoPartners() {
    ...
    if (filtered.length !== partners.length) {
        ...
        debugLog('DEMO', `Partners: Removed ${partners.length - filtered.length} demo partners`);
    }
}
...
function savePartnersOverride(data) {
    try {
        ...
        const teamMembers = existingTeam.filter(item => !item.logo || !item.website);
...
const SEED_DATA = {
    games: [
        ...
        {
            title: "Elden Ring",
            description: "Action-RPG ouvert créé par FromSoftware et George R.R. Martin.",
            image: "https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202107/1612/Y5RHNmzAtc6sRYwZlZWkgOWJ.png",
            platforms: ["PC", "PS5", "Xbox Series X"],
            genres: ["Action", "RPG"],
            rating: 4.7
        ...
        {
            title: "Spider-Man 2",
            description: "Nouvelle aventure de Spider-Man avec Peter Parker et Miles Morales.",
            image: "https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1219/1c2d8c1c87ab30eca44e72dc8741cd5d24ea85a8cfdb2f95.png",
            platforms: ["PS5"],
            genres: ["Action", "Aventure"],
            rating: 4.6
        },
        {
            ...
            description: "Remake du classique survival horror avec des graphismes modernes.",
            image: "https://cdn.cloudflare.steamstatic.com/steam/apps/2050650/capsule_616x353.jpg",
            platforms: ["PC", "PS5", "Xbox Series X"],
    ...
    articles: {
        actu: [
            ...
            },
            {
                title: "Steam Deck OLED : Une mise à jour réussie",
                excerpt: "Valve améliore sa console portable avec un écran OLED et une meilleure autonomie.",
                cover: "https://cdn.cloudflare.steamstatic.com/steamcommunity/public/images/clans/39049601/8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b8b.jpg",
                category: "actu"
            },
            {
                title: "Baldur's Gate 3 : Record de ventes battu",
            ...
            },
            {
                title: "Ubisoft : Nouveaux studios en développement",
                excerpt: "L'éditeur français annonce l'ouverture de studios en Suède et en Espagne.",
                cover: "https://staticctf.ubisoft.com/J3yJr34U2pZ2Ieem48Dwy69uqbjxBSd33xIvvzf2K9Q/prismic/field/image/Z2V0SW5Ub3VjaFdpdGhHYW1pbmd8ZWFhZWJiNjMtODM3ZC00NzY4LWI2ZGYtNmM5MmM3ZmM2MzM2fGVhLWFzc2V0cy1pbWFnZS1oZWFkZXI=",
                category: "actu"
            }
        ],
    ...
    tests: [
        ...
        },
        {
            title: "Avis Baldur's Gate 3 : Le RPG ultime",
            excerpt: "Un chef-d'œuvre tactique qui redéfinit le genre RPG moderne.",
            content: "Baldur's Gate 3 est le jeu que tout amateur de RPG attendait...",
            coverUrl: "https://cdn.cloudflare.steamstatic.com/steam/apps/1086940/capsule_616x353.jpg",
            gameId: "baldurs-gate-3",
            score: 9.7,
            criteria: [
        ...
        },
        {
            title: "Critique God of War Ragnarök",
            excerpt: "Une conclusion épique à la saga de Kratos et Atreus.",
            content: "God of War Ragnarök achève magnifiquement la nouvelle trilogie...",
            coverUrl: "https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202207/1210/4xJ8XB3bi888QTLZYdl7Oi0.png",
            gameId: "god-of-war-ragnarok",
            score: 9.2,
            criteria: [
        ...
        },
        {
            title: "Avis Spider-Man 2 : Un retour réussi",
            excerpt: "Marvel's Spider-Man 2 confirme l'excellence de la série sur PS5.",
            content: "Après un premier volet déjà excellent, Spider-Man 2 élève encore la barre...",
            coverUrl: "https://cdn.akamai.steamstatic.com/vulcan/ap/rnd/202306/1219/1c2d8c1c87ab30eca44e72dc8741cd5d24ea85a8cfdb2f95.png",
            gameId: "spider-man-2",
            score: 8.9,
            criteria: [
        ...
        },
        {
            title: "Critique Cyberpunk 2077 : Phantom Liberty",
            excerpt: "L'extension rachète les défauts du jeu de base.",
            content: "Phantom Liberty transforme Cyberpunk 2077 en l'expérience qu'il aurait dû être...",
            coverUrl: "https://cdn.cloudflare.steamstatic.com/steam/apps/1091500/capsule_616x353.jpg",
            gameId: "cyberpunk-2077",
            score: 8.5,
            criteria: [
        ...
        },
        {
            title: "Test Hogwarts Legacy",
            excerpt: "Un RPG magique qui ravira les fans d'Harry Potter.",
            content: "Avalanche Software livre une adaptation réussie de l'univers Harry Potter...",
            coverUrl: "https://cdn.cloudflare.steamstatic.com/steam/apps/990080/capsule_616x353.jpg",
            gameId: "hogwarts-legacy",
            score: 8.7,
            criteria: [
...
function seedGames() {
    ...
    if (toAdd === 0) {
        debugLog('SEED', `Games: Already have ${existingGames.length}/${targetCount}`);
        return;
    }
    ...
    debugLog('SEED', `Games: Added ${gamesToAdd.length} games (total: ${allGames.length})`);
}
...
function seedArticles() {
    const existingArticles = getArticlesData();
    const actuExisting = existingArticles.filter(a => a.category === 'actu').length;
    const guidesExisting = existingArticles.filter(a => a.category === 'guide').length;
    ...
    // Seed actu
    if (actuToAdd > 0) {
        const actuToSeed = SEED_DATA.articles.actu.slice(0, actuToAdd).map(article => ({
            ...article,
    ...
    // Seed guides
    if (guidesToAdd > 0) {
        const guidesToSeed = SEED_DATA.articles.guides.slice(0, guidesToAdd).map(article => ({
            ...article,
            id: generateId(existingArticles, slugify(article.title)),
            slug: slugify(article.title),
            publishedAt: new Date().toISOString(),
    ...
    if (addedArticles > 0) {
        ...
        saveArticlesOverride(allArticles);
        debugLog('SEED', `Articles: Added ${addedArticles} articles (${actuToSeed.length} actu + ${guidesToSeed.length} guides)`);
    } else {
        debugLog('SEED', `Articles: Already have sufficient content (${actuExisting} actu + ${guidesExisting} guides)`);
    }
}
...
function seedTests() {
    ...
    if (toAdd === 0) {
        debugLog('SEED', `Tests: Already have ${existingTests.length}/${targetCount}`);
        return;
    }
    ...
    debugLog('SEED', `Tests: Added ${testsToAdd.length} tests (total: ${allTests.length})`);
}
...
function seedBonsPlans() {
    ...
    if (toAdd === 0) {
        debugLog('SEED', `Bons Plans: Already have ${existingBonsPlans.length}/${targetCount}`);
        return;
    }
    ...
    debugLog('SEED', `Bons Plans: Added ${bonsPlansToAdd.length} deals (total: ${allBonsPlans.length})`);
}
...
function seedPromos() {
    ...
    if (simulatedAdd > 0) {
        ...
        debugLog('SEED', `Promos: Simulated ${simulatedAdd} promos (sidebar feature)`);
    }
}
...
function seedPartners() {
    ...
    if (simulatedAdd > 0) {
        ...
        debugLog('SEED', `Partners: Simulated ${simulatedAdd} partnerships (footer carousel)`);
    }
}
...
function performAudit() {
    ...
    articles.forEach(article => {
        ...
        if (article.externalUrl && !article.externalUrl.startsWith('http')) {
            auditResults.articles.push(`Article "${article.title}": invalid external URL`);
        }

        // Vérifier les images
        if (!article.cover || !article.cover.startsWith('http')) {
            auditResults.articles.push(`Article "${article.title}": missing or invalid cover image`);
        }

        // Vérifier les slugs dupliqués
        const duplicates = articles.filter(a => a.slug === article.slug && a.id !== article.id);
        if (duplicates.length > 0) {
            auditResults.articles.push(`Article "${article.title}": duplicate slug "${article.slug}"`);
        }
    });
    ...
    tests.forEach(test => {
        if (!test.gameId) {
            ...
            const game = games.find(g => g.id === test.gameId);
            if (!game) {
                auditResults.tests.push(`Test "${test.title}": references non-existent game "${test.gameId}"`);
            }
        }
    ...
    bonsPlans.forEach(bonPlan => {
        if (!bonPlan.externalUrl || !bonPlan.externalUrl.startsWith('http')) {
            auditResults.bonsPlans.push(`Bon plan "${bonPlan.title}": missing or invalid external URL`);
        }

        if (!bonPlan.coverUrl || !bonPlan.coverUrl.startsWith('http')) {
            auditResults.bonsPlans.push(`Bon plan "${bonPlan.title}": missing or invalid cover`);
        }
    });

    window.debugState.auditResults = auditResults;
    debugLog('AUDIT', `Audit completed: ${Object.values(auditResults).flat().length} issues found`);
    updateDebugPanel();
}
...
function updateDebugOverlay() {
    ...
    if (content && window.debugBuffer) {
        content.innerHTML = window.debugBuffer.map(entry => `
            <div style="margin-bottom: 4px; border-left: 2px solid ${
                entry.scope === 'LAYOUT' ? '#4ade80' :
                entry.scope === 'DATA' ? '#fbbf24' :
                entry.scope === 'ERROR' ? '#ef4444' : '#666'
            }; padding-left: 6px;">
...
function diagnoseData() {
    ...
    const dataInfo = {
        ...
        bonsPlans: window.DEALS || window.BONS_PLANS ? (window.DEALS || window.BONS_PLANS).length : 0,
    ...
    debugLog('DATA', {
        ...
        overrides: {
            ...
            articles: localStorage.getItem('lermite_articles_override') ? 'present' : 'none',
            tests: localStorage.getItem('lermite_tests_override') ? 'present' : 'none',
            bonsPlans: localStorage.getItem('lermite_bonsplans_override') ? 'present' : 'none',
            promos: localStorage.getItem('lermite_promos_override') ? 'present' : 'none',
            sponsors: localStorage.getItem('lermite_sponsors_override') ? 'present' : 'none',
...
function createBonPlanBlockElement(block, index) {
    ...
    blockEl.dataset.blockId = block.blockId || `block-${Date.now()}-${index}-${Math.random().toString(36).substr(2, 9)}`;
    blockEl.style.cssText = 'background: rgba(31, 42, 68, 0.3); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--spacing-md); margin-bottom: var(--spacing-md);';
    ...
    if (block.type === 'text') {
        ...
        const levelNum = typeof block.level === 'number' ? block.level : (block.level === 'h3' ? 3 : 2);
        content = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-sm);">
...
function moveBonPlanBlock(index, direction) {
    if (direction === 'up' && index > 0) {
        [bonPlanBlocks[index], bonPlanBlocks[index - 1]] = [bonPlanBlocks[index - 1], bonPlanBlocks[index]];
        renderBonPlanBlocks(bonPlanBlocks);
...
function ensureTestModal() {
    ...
    if (!modal) {
        console.warn('[DEBUG] Modal test non trouvé, vérifiez que admin.html contient le modal');
...
function populateTestForm(refs, test) {
    ...
    setVal(refs.publishedAt, test.publishedAt ? new Date(test.publishedAt).toISOString().slice(0, 16) : '');
    ...
    if (contentBlocks && Array.isArray(contentBlocks) && contentBlocks.length > 0) {
        ...
        const blocksWithId = contentBlocks.map((block, idx) => ({
            ...block,
            blockId: block.blockId || `block-${Date.now()}-${idx}-${Math.random().toString(36).substr(2, 9)}`
        }));
...
function moveTestBlock(index, direction) {
    if (direction === 'up' && index > 0) {
        [testBlocks[index], testBlocks[index - 1]] = [testBlocks[index - 1], testBlocks[index]];
        renderTestBlocks(testBlocks);
...
function renderTestBlocksToHtml(blocks) {
    ...
    blocks.forEach(block => {
        ...
        } else if (block.type === 'image' || block.type === 'img' || block.type === 'media') {
            const src = (block && typeof block.src === 'string' && block.src.trim()) ||
...
function fillBonPlanForm(bonPlan) {
    ...
    document.getElementById('bon-plan-external-url').value = bonPlan.externalUrl || '';
    ...
    document.getElementById('bon-plan-discount').value = bonPlan.discountPercent || '';
    document.getElementById('bon-plan-publishedAt').value = bonPlan.publishedAt ? new Date(bonPlan.publishedAt).toISOString().slice(0, 16) : '';
    document.getElementById('bon-plan-isFeatured').checked = bonPlan.isFeatured || false;
    document.getElementById('bon-plan-openInNewTab').checked = bonPlan.openInNewTab !== false;
...
function normalizeBonPlanPayload(formData) {
    ...
    if (!slug && title) {
        ...
        const existingBonsPlans = getBonsPlansData().filter(bp => bp.id !== finalId);
        let uniqueSlug = slug;
        let counter = 1;
    ...
    return {
        ...
        discountPercent: formData.get('discountPercent') ? parseInt(formData.get('discountPercent')) : null,
...
function renderAdminBonsPlansList() {
    ...
    if (bonsPlans.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--color-text-secondary);">Aucun bon plan</p>';
        return;
    }

    container.innerHTML = bonsPlans.map(bonPlan => `
...
function initTestPage() {
    ...
    const gameName = game ? game.title : (test.gameTitle || 'Jeu inconnu');
    const platforms = game ? (game.platforms || []).join(', ') : 'Toutes plateformes';
    ...
    const publishedDate = test.publishedAt ?
        ...
        const criteriaHtml = test.criteria && Array.isArray(test.criteria) && test.criteria.length > 0 ?
...
} else if (document.querySelector('.content') && (window.location.pathname.includes('articles/') || document.querySelector('article.page-hero'))) {
...
window.applyGameView = function() {
    ...
    const isJeuxPage = window.location.pathname.includes('jeux.html') || document.querySelector('#games-grid');
...
function renderGames(games, containerId = 'gamesGrid', options = {}) {
    ...
    // Debug visuel
    if (window.DEBUG_GRID) {
        window.addGridDebug(container, `GAMES-${containerId}`, games.length, options.enablePagination !== false);
    }
...
function renderAdminGamesList() {
    ...
    if (!list) {
        console.warn('adminGamesList container not found - retrying in next frame...');
    ...
    if (!games || !Array.isArray(games) || !games.length) {
        list.innerHTML = '<div class="empty-state">Aucun jeu chargé (data.js non lu)</div>';
        return;
    }
...
function initAdmin() {
    ...
    const isAdminPage = document.body && document.body.dataset && document.body.dataset.page === 'admin';
    if (!isAdminPage) {
        return; // Pas sur admin, ne rien faire
    }
    ...
    try {
        ...
        if (list) {
            list.innerHTML = '<div class="empty-state" style="color: #fca5a5;">Erreur chargement data.js — vérifie le chemin ./data.js</div>';
        }
    }
}
...
function saveGamesOverride(nextGames) {
    try {
        localStorage.setItem(window.LMD.storageKeys.games, JSON.stringify(nextGames));
    } catch (e) {
...
function normalizeGame(game) {
    ...
    const normalized = {
        ...
        platforms: Array.isArray(game.platforms) ? game.platforms.map(p => toText(p)).filter(p => p) : [],
        genres: Array.isArray(game.genres) ? game.genres.map(g => toText(g)).filter(g => g) : [],
        modes: Array.isArray(game.modes) ? game.modes.map(m => toText(m)).filter(m => m) : undefined,
...
function saveArticlesOverride(nextArticles) {
    try {
                ...
                try {
                    ...
                    if (Array.isArray(blocks)) {
                        const img = blocks.find(b => (
                            (b && (b.type === "image" || b.type === "img" || b.type === "media")) &&
                            (b.display || b.mode)
                        ));
                        if (img) imgDisplay = img.display || img.mode || null;
                    }
                    console.debug("[admin][storage] setItem key=", key);
                    console.debug("[admin][storage] first keys=", first ? Object.keys(first) : null);
                    console.debug("[admin][storage] blocks?", Array.isArray(blocks) ? blocks.length : null, "imgDisplay=", imgDisplay);
    } catch (e) {
                    console.debug("[admin][storage] setItem key=", key, "(debug failed)", e);
                }
        ...
        if (isQuotaError) {
            ...
            alert('Stockage plein : l\'image est trop lourde. Utilise plutôt une URL (https) ou une image plus légère.');
    ...
    hydratedArticles.forEach(article => {
        ...
        if (!article.link) {
            if (article.cta && article.cta.type && article.cta.type !== 'none') {
                ...
                const affiliateUrl = article.affiliateUrl || article.affiliateLink || '';
        ...
        if (!article.link) {
            article.link = { type: 'none', url: '', title: '', text: '', buttonText: '', discount: '' };
        } else {
...
function saveTestsOverride(nextTests) {
    try {
        ...
        if (isAdminPage) {
            function __debugSetItem(key, value) {
                try {
                    ...
                    const first = Array.isArray(preview) ? preview[0] : preview;
                    console.debug("[admin][tests] setItem first test=", first ? {id: first.id, title: first.title, score: first.score} : null);
                } catch (e) {
        ...
        if (isQuotaError) {
            console.error('Erreur quota localStorage lors de la sauvegarde des tests. Veuillez libérer de l\'espace.');
            alert('Erreur de sauvegarde : quota localStorage dépassé. Essayez de supprimer des données ou videz le cache du navigateur.');
        } else {
            console.error('Erreur lors de la sauvegarde des tests:', e);
            alert('Erreur lors de la sauvegarde des tests. Vérifiez la console pour plus de détails.');
        }
        return false;
    }
}
...
function getTestsData() {
    ...
    hydratedTests.forEach(test => {
        ...
        if (test.criteria && Array.isArray(test.criteria) && test.criteria.length > 0) {
            const validScores = test.criteria
                .map(c => typeof c.score === 'number' ? c.score : 0)
                .filter(s => s >= 0 && s <= 10);
            if (validScores.length > 0) {
                test.score = validScores.reduce((a, b) => a + b, 0) / validScores.length;
            }
        }
...
function saveContactRequests(requests) {
    try {
        localStorage.setItem(window.LMD.storageKeys.contactRequests, JSON.stringify(requests));
        return true;
    } catch (e) {
...
function saveConditionsText(text) {
    try {
        localStorage.setItem(window.LMD.storageKeys.conditions, JSON.stringify(text));
        return true;
    } catch (e) {
...
function saveAuthUsers(users) {
    try {
        localStorage.setItem(window.LMD.storageKeys.authUsers, JSON.stringify(users));
        return true;
    } catch (e) {
...
function saveAuthSession(user) {
    try {
        localStorage.setItem(window.LMD.storageKeys.authSession, JSON.stringify(user));
        return true;
    } catch (e) {
        return false;
    }
...
function signupUser(userData) {
    ...
    // Vérifications
    if (users.find(u => u.pseudo.toLowerCase() === userData.pseudo.toLowerCase())) {
...
function initSignupPage() {
    ...
    signupForm.addEventListener('submit', async (e) => {
        ...
        // Validation
        if (!userData.pseudo || !userData.email || !userData.password || !userData.passwordConfirm) {
            showFormError('signup-pseudo', 'Tous les champs sont requis');
            return;
        }

        if (userData.pseudo.length < 3) {
            showFormError('signup-pseudo', 'Le pseudo doit contenir au moins 3 caractères');
            return;
        }
        ...
        if (userData.password.length < 6) {
            showFormError('signup-password', 'Le mot de passe doit contenir au moins 6 caractères');
            return;
        }

        if (userData.password !== userData.passwordConfirm) {
            showFormError('signup-password-confirm', 'Les mots de passe ne correspondent pas');
            return;
        }
...
function formatConditionsText(text) {
    ...
    return paragraphs.map(p => {
        ...
        if (/^\d+\./.test(trimmed) || /^(Objet|Accès|Contenus|Responsabilité|Liens|Propriété|Signalement|Modification|Droit)/i.test(trimmed)) {
...
function renderBonsPlansPage() {
    ...
    function renderBonPlanItem(bonPlan) {
        ...
        } else if (bonPlan.oldPrice && bonPlan.price && bonPlan.oldPrice > bonPlan.price) {
        ...
        // Image de couverture
        const imageHtml = bonPlan.coverUrl && bonPlan.coverUrl.trim()
...
function getBonsPlansData() {
    ...
    hydratedBonsPlans.forEach(bonPlan => {
        ...
        if (typeof bonPlan.discountPercent !== 'number') bonPlan.discountPercent = null;
...
function saveBonsPlansOverride(data) {
    try {
        localStorage.setItem(window.LMD.storageKeys.bonsplans, JSON.stringify(data));
...
function getSponsorsData() {
    ...
    console.log('[DEBUG] getSponsorsData - key:', window.LMD.storageKeys.sponsors, 'raw:', raw);
    if (!raw) {
...
function normalizeSponsorPayload(formData) {
    ...
    const id = formData.get('id') || `spon_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
...
function pickActiveSponsor(items, now = new Date()) {
    ...
    for (const item of sortedItems) {
        console.log('[DEBUG] Checking sponsor:', item.title, 'active:', item.active);
        if (!item.active) {
        ...
        console.log('[DEBUG] Dates - startAt:', startAt, 'endAt:', endAt, 'now:', now);
...
function closeTestModal() {
    ...
    if (modal) {
        ...
        const previewPlaceholder = document.getElementById('testCoverPreviewPlaceholder');
...
function renderTestCriteria(criteria) {
    ...
    if (criteria.length === 0) {
        container.innerHTML = '<div class="criteria-empty">Aucun critère défini</div>';
        return;
    }
...
function clearTestFormErrors() {
    const inputs = document.querySelectorAll('#testForm .invalid, #testForm .is-invalid');
...
function saveMediaData(mediaList) {
    try {
        localStorage.setItem(window.LMD.storageKeys.media, JSON.stringify(mediaList));
    } catch (e) {
        // Fallback silencieux (zéro console error)
...
function saveMediaFolders(folders) {
    try {
        localStorage.setItem(window.LMD.storageKeys.mediaFolders, JSON.stringify(folders));
    } catch (e) {
        // Fallback silencieux (zéro console error)
    }
}
...
function initAdminPage() {
    ...
    console.log('initAdminPage: Event listeners pour blocs articles seront installés'); // DEBUG
    ...
    if (btnReset) {
        btnReset.addEventListener('click', function() {
            if (confirm('Êtes-vous sûr de vouloir réinitialiser ? Tous les jeux modifiés/ajoutés seront perdus.')) {
    ...
    function setAdminTab(key) {
        ...
        const sections = Array.from(document.querySelectorAll('.admin-section[data-admin-section]'));
        ...
        if (!finalSection) {
            console.warn('[ADMIN TABS] missing section for key', key, '- fallback to games');
            finalKey = 'games';
        ...
        adminTabsContainer.addEventListener('click', (e) => {
            ...
            if (tabKey) {
                ...
                if (tabKey === 'promo') {
                    ...
                    if (aboutTitleInput) aboutTitleInput.value = teamData.aboutTitle || 'Équipe';
                    if (aboutTextInput) aboutTextInput.value = teamData.aboutText || 'Les admins et rédacteurs du site';
                }
            }
        });
    }
    ...
    if (authorIdSelect) {
        ...
        const authors = activeMembers.length > 0 ? activeMembers.map(m => {
            const normalized = normalizeTeamMember(m);
            return {
                id: normalized.id,
                name: normalized.name,
        ...
        if (defaultOption) {
            ...
            authorIdSelect.innerHTML = '<option value="">-- Sélectionner un auteur --</option>';
        }
        // Ajouter les auteurs
    ...
    if (articleForm) {
        ...
        const linkButtonHelper = document.getElementById('article-link-button-helper');
        
        if (ctaTypeSelect && ctaUrlInput && linkTitleInput && linkTextInput && linkButtonInput && linkDiscountInput) {
            function toggleCtaFields() {
                ...
                if (type === 'none') {
                    ...
                    if (type === 'youtube') {
                        ctaUrlInput.placeholder = 'https://youtube.com/... ou https://youtu.be/...';
                    } else {
        ...
        blocksListContainer.addEventListener('click', function(e) {
            ...
            if (pickMediaBtn) {
                ...
                if (!isNaN(index)) {
                    ...
                    const blocksList = document.getElementById('article-blocks-list');
                    if (blocksList) {
                        const blockEl = blocksList.querySelector(`[data-block-index="${index}"]`);
                        if (blockEl) {
                            const urlInput = blockEl.querySelector('input[data-block-field="url"]');
                            if (urlInput) {
            ...
            const pickMediaOfferBtn = e.target.closest('[data-block-pick-media-offer]');
            if (pickMediaOfferBtn) {
                ...
                if (!isNaN(index)) {
                    ...
                    const blockEl = pickMediaOfferBtn.closest('.article-block-item') || pickMediaOfferBtn.closest('[data-block-id]') || pickMediaOfferBtn.closest('.block');
                        if (blockEl) {
        ...
        blocksListContainer.addEventListener('input', function(e) {
            ...
            if (urlInput) {
                ...
                if (blockEl) {
                    ...
                    const previewEl = blocksListContainer.querySelector(`[data-block-preview="${index}"]`);
                    if (previewEl) {
                        ...
                        if (isImageSrcPreviewable(url)) {
                            ...
                            const altInput = blockEl.querySelector('input[data-block-field="alt"]');
                            ...
                            previewEl.innerHTML = '<div style="color: var(--color-text-secondary); font-size: 0.75em;">Aperçu indisponible</div>';
                        }
                    }
                }
            }
                ...
                if (blockEl && blockEl.dataset.blockType === 'offer') {
                    ...
                    if (previewEl) {
                        if (isImageSrcPreviewable(imageUrl)) {
                            const titleInput = blockEl.querySelector('input[data-field="title"]');
                            const title = titleInput ? escapeHtml(titleInput.value || '') : '';
            ...
            if (youtubeUrlInput) {
                ...
                if (blockEl && blockEl.dataset.blockType === 'youtube') {
                    ...
                    console.log('[YT DEBUG PREVIEW] rawUrl=', url, 'id=', videoId, 'embedSrc=', embedSrc, 'isFileProtocol=', isFileProtocol);
                    ...
                    if (previewEl) {
                        if (videoId) {
                            ...
                            previewEl.innerHTML = '<div style="color: var(--color-text-secondary); font-size: 0.75em; text-align: center; padding: var(--spacing-md);">URL YouTube invalide</div>';
                        }
                    }
                }
            }
                        ...
                        if (url) {
                            ...
                            function ensureTwitterWidgets(cb) {
                                ...
                                if (document.querySelector('script[data-twitter-widgets]')) {
                                    const t = setInterval(() => {
                                        if (window.twttr && window.twttr.widgets) { clearInterval(t); cb(); }
                                    }, 50);
                                    return;
                                }
                            ...
                            previewEl.innerHTML = '<div style="color: var(--color-text-secondary); font-size: 0.75em; text-align: center; padding: var(--spacing-md);">Aperçu indisponible</div>';
                        }
                    }
                }
            }
        ...
        testBlocksListContainer.addEventListener('click', function(e) {
            ...
            if (pickMediaBtn) {
                ...
                if (!isNaN(index)) {
                    ...
                    const blockEl = testBlocksListContainer.querySelector(`[data-block-index="${index}"]`);
                    if (blockEl) {
                        const urlInput = blockEl.querySelector('input[data-block-field="url"]');
                        if (urlInput) {
                            // Mémoriser la référence de l'input ciblé
    ...
    const adminContactRequestsList = document.getElementById('adminContactRequestsList');
    ...
    if (btnInjectDemo) {
        btnInjectDemo.addEventListener('click', () => {
            if (confirm('Cette action va injecter du contenu démo réaliste dans localStorage.\n\nToutes les pages seront remplies après refresh.\n\nContinuer ?')) {
                ...
                setTimeout(() => {
                    ...
                    renderAdminBonsPlansList();
                    updateAdminSiteCounters();
                    alert('Contenu démo injecté ! Rafraîchissez les pages publiques pour voir le résultat.');
                }, 500);
            }
        });
    }
    ...
    if (btnResetDemo) {
        btnResetDemo.addEventListener('click', () => {
            if (confirm('Cette action va supprimer UNIQUEMENT le contenu démo injecté.\nLe contenu réel créé manuellement sera conservé.\n\nContinuer ?')) {
    ...
    if (btnCancelBug) btnCancelBug.addEventListener('click', closeBugModalHandler);
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && bugModal && bugModal.classList.contains('is-open')) {
            closeBugModal();
        }
    });
    ...
    setTimeout(() => {
        try {
            ...
            const validTabs = ['games', 'articles', 'tests', 'bons-plans', 'media', 'bugs', 'contact', 'conditions', 'users'];
    ...
    const btnChooseMediaBonPlan = document.getElementById('btn-choose-media-bon-plan');
    ...
    if (btnAddCriterion) {
        btnAddCriterion.addEventListener('click', () => {
            ...
            const criteriaInputs = document.querySelectorAll('#test-criteria-list .criterion-item');
    ...
    const bonPlanBlocksListContainer = document.getElementById('bon-plan-blocks-list');
    if (bonPlanBlocksListContainer) {
        bonPlanBlocksListContainer.addEventListener('click', function(e) {
            ...
            if (pickMediaBtn) {
                ...
                if (!isNaN(index)) {
                    // Mémoriser la référence directe de l'input pour injection fiable
                    const blockEl = bonPlanBlocksListContainer.querySelector(`[data-block-index="${index}"]`);
                    if (blockEl) {
                        const urlInput = blockEl.querySelector('input[data-block-field="url"]');
                        if (urlInput) {
                            // Mémoriser la référence de l'input ciblé
    ...
    const initActiveTab = () => {
        ...
        if (window.location.hash) {
            ...
            const validTabs = ['games', 'articles', 'tests', 'bons-plans', 'media', 'promo', 'partnership', 'team'];
    ...
    const btnChooseMediaArticle = document.getElementById('btn-choose-media-article');
    ...
    if (btnChooseMediaArticle) {
        btnChooseMediaArticle.addEventListener('click', () => openMediaPicker('article'));
    }
    ...
    const btnCloseMediaPicker = document.getElementById('mediaPickerModalCloseBtn');
    ...
    document.addEventListener('click', function(e) {
        ...
        if (mediaListGrid && !e.target.closest('.media-menu') && !e.target.closest('[data-media-menu-btn]')) {
            closeAllMediaMenus();
        }
        
        // Si le clic n'est pas dans un menu ou un bouton kebab (dossiers), fermer les menus dossiers
        if (foldersGrid && !e.target.closest('.folder-menu') && !e.target.closest('[data-folder-menu-btn]')) {
            closeAllFolderMenus();
        }
    });
    ...
    const mediaPickerFolderFilter = document.getElementById('mediaPickerFolderFilter');
            ...
            if (url && window.currentMediaTarget) {
                ...
                if (target === 'game') {
                    ...
                    if (window.__mediaPickerTarget) {
                        ...
                        if (window.__mediaPickerTarget.target && !window.__mediaPickerTarget.blockEl) {
                    ...
                    if (window.__mediaTargetAvatarImage) {
                        const avatarImageUrlInput = document.getElementById('team-member-avatar-image-url');
                        ...
                        if (avatarImageUrlInput) {
                            ...
                            avatarImageUrlInput.dispatchEvent(new Event('input', { bubbles: true }));
                    ...
                    if (window.__mediaPickerTarget && window.__mediaPickerTarget.target && !window.__mediaPickerTarget.blockEl) {
                    ...
                    if (window.__mediaPickerTarget && window.__mediaPickerTarget.blockEl) {
                        ...
                        console.log('[admin] image replaced (offer)', { blockId: blockEl.dataset.blockId || 'unknown', url: chosenValue.substring(0, 50) + '...', inputValue: realTarget?.value?.substring(0, 50) + '...' });
                    ...
                    const urlInput = window.__mediaTargetInput;
                        ...
                        console.log('[admin] image replaced', { blockId: blockId || 'unknown', blockIndex, url: chosenValue.substring(0, 50) + '...', inputValue: urlInput.value.substring(0, 50) + '...' });
                    ...
                    const avatarMediaIdInput = document.getElementById('team-member-avatar-media-id');
                    const avatarTypeSelect = document.getElementById('team-member-avatar-type');
    ...
    const btnCloseRenameFolder = document.getElementById('renameFolderModalCloseBtn');
    const overlayRenameFolder = document.getElementById('renameFolderModalOverlay');
    ...
    if (btnConfirmMoveMedia) {
        btnConfirmMoveMedia.addEventListener('click', function() {
            ...
            if (select && currentMoveMediaId) {
                ...
                const mediaList = getMediaData();
                const media = mediaList.find(m => toText(m.id) === currentMoveMediaId);
                const currentFolder = media ? (toText(media.folder) || 'Non classé') : 'Non classé';
        ...
        if (select) {
            select.addEventListener('change', function() {
                ...
                btnConfirmMoveMedia.style.cursor = isCurrent ? 'not-allowed' : 'pointer';
            });
        }
    }
...
function renderMediaFoldersView() {
    ...
    foldersGrid.innerHTML = folders.map(folder => {
        ...
        const count = mediaList.filter(m => (toText(m.folder) || 'Non classé') === folder).length;
        ...
        return `
            <div class="media-folder-card" data-folder-name="${folder}" data-folder="${folder}" style="background: rgba(31, 42, 68, 0.3); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--spacing-md); cursor: pointer; transition: all var(--transition-base); position: relative;" onmouseover="this.style.background='rgba(31, 42, 68, 0.5)'; this.style.borderColor='rgba(59,130,246,0.3)';" onmouseout="this.style.background='rgba(31, 42, 68, 0.3)'; this.style.borderColor='var(--color-border)';">
                ...
                ${!isNonClasse ? `<div class="folder-menu" data-folder-menu="${folder}" aria-hidden="true">
    ...
    foldersGrid.addEventListener('drop', function(e) {
        ...
        if (folderCard) {
            ...
            const folderName = folderCard.dataset.folder || folderCard.dataset.folderName;
            const mediaId = e.dataTransfer.getData('text/plain') || draggedMediaId;
...
function renderMediaFolderContent(folder) {
    ...
    mediaListGrid.style.gridTemplateColumns = 'repeat(auto-fill, minmax(280px, 1fr))';
    ...
    const mediaList = getMediaData();
    const folderMedia = mediaList.filter(m => (toText(m.folder) || 'Non classé') === folder);
    
    if (folderMedia.length === 0) {
        mediaListGrid.innerHTML = '<div class="empty-state">Aucun média dans ce dossier</div>';
        return;
    }
    ...
    mediaListGrid.innerHTML = folderMedia.map(media => {
        const urlTruncated = media.url.length > 50 ? media.url.substring(0, 50) + '...' : media.url;
    ...
    if (!mediaListGrid.dataset.hasMediaMenuListener) {
        ...
        mediaListGrid.addEventListener('click', function(e) {
            ...
            if (kebabBtn) {
                e.preventDefault();
                e.stopPropagation();
                const mediaId = kebabBtn.dataset.mediaMenuBtn || kebabBtn.getAttribute('data-media-menu-btn');
                if (mediaId) {
...
function deleteMedia(id) {
    ...
    if (isUsed) {
        ...
        const parts = [];
        if (gameCount > 0) parts.push(`${gameCount} jeu${gameCount > 1 ? 'x' : ''}`);
...
function showMediaDropZone() {
    ...
    dropZoneList.innerHTML = folders.map(folder => {
        return `
...
function showDropBar() {
    ...
    dropBarTargets.innerHTML = folders.map(folder => {
...
function validateRenameFolderInput(newName, oldName) {
    ...
    const folders = getAllMediaFolders();
    const exists = folders.some(f => f.toLowerCase() === trimmed.toLowerCase() && f !== oldName);
...
function deleteFolder(folderName) {
    ...
    if (!confirm(`Supprimer le dossier "${folderName}" ? Les médias seront déplacés vers "Non classé".`)) {
        return;
    }
...
function handleMediaListClick(e) {
    ...
    if ((!list || !list.contains(e.target)) && (!mediaListGrid || !mediaListGrid.contains(e.target))) {
        return;
    }
...
function renderMediaPickerList() {
    ...
    if (mediaList.length === 0) {
        pickerList.innerHTML = '<div class="empty-state">Aucun média disponible' + (searchTerm ? ' pour "' + searchTerm + '"' : '') + '</div>';
    } else {
        pickerList.innerHTML = mediaList.map(media => {
            const url = toText(media.url);
            const id = toText(media.id);
            const folder = toText(media.folder) || 'Non classé';
            return `
    ...
    try {
        ...
        if (payload.category === 'test') {
            showArticleFormErrors({
                'article-category': 'La catégorie Tests n\'est plus disponible. Utilise la section Tests dédiée.'
            });
            console.error('[admin] Tentative de sauvegarde d\'article avec catégorie "test" bloquée');
            return;
        }

        // PATCH C: DEBUG PROUVE TOUT DE SUITE - afficher imageUrl du bloc offre dans le payload
        const offerDebug = (payload.contentBlocks || payload.blocks || []).find(b => {
            const t = (b.type || '').toLowerCase();
            return t === 'offer' || t === 'affiliation';
        });
        console.log('[DEBUG payload offer imageUrl]', offerDebug?.imageUrl, offerDebug);
        ...
        const saved = getArticlesData().find(a => a.id === payload.id) || getArticlesData().find(a => a.slug === payload.slug);
        const savedOffer = (saved?.contentBlocks || saved?.blocks || []).find(b => ((b.type||'').toLowerCase()==='offer' || (b.type||'').toLowerCase()==='affiliation'));
...
function renderArticlesListingPage(options = {}) {
    ...
    if (categoryKey) {
        ...
        const normalizedExpected = expectedCategories.map(c => normalizeCategory(c));
    ...
    function renderArticleItem(article) {
        ...
        let imageHtml = '';
        if (article.cover && article.cover.trim() && (article.cover.startsWith('http://') || article.cover.startsWith('https://'))) {
...
function initSectionArticlesPage(options = {}) {
    ...
    function filterArticles(articles) {
        return articles.filter(article => {
            ...
            if (selectedConsoles.length > 0 && linkedGame.platforms) {
                const gamePlatforms = linkedGame.platforms.map(p => normalizeForMatch(p));
                consoleMatch = selectedConsoles.some(selectedChip => {
                    const possibleMatches = consoleMapping[selectedChip] || [selectedChip];
                    return gamePlatforms.some(platform => 
                        possibleMatches.some(match => normalizeForMatch(platform).includes(normalizeForMatch(match)) || normalizeForMatch(match).includes(normalizeForMatch(platform)))
                    );
                });
            }
            ...
            if (selectedGenres.length > 0 && linkedGame.genres) {
                ...
                genreMatch = selectedGenres.some(selectedChip => {
                    ...
                    return gameGenres.some(genre => 
                        possibleMatches.some(match => normalizeForMatch(genre).includes(normalizeForMatch(match)) || normalizeForMatch(match).includes(normalizeForMatch(genre)))
                    );
                });
            }
    ...
    function render() {
        ...
        // Debug visuel
        if (window.DEBUG_GRID) {
            window.addGridDebug(container, 'ARTICLES-articlesGrid', filtered.length, true);
        }
        ...
        setTimeout(() => {
            const gridEl = document.getElementById('articlesGrid') || document.querySelector('.articles-grid');
...
function validateGame(payload) {
    ...
    if (payload.status && !['released', 'early-access', 'upcoming'].includes(payload.status)) {
...
function fillForm(game) {
    ...
    document.getElementById('game-platforms').value = Array.isArray(game.platforms) ? game.platforms.join(', ') : toText(game.platforms);
    document.getElementById('game-genres').value = Array.isArray(game.genres) ? game.genres.join(', ') : toText(game.genres);
    document.getElementById('game-status').value = game.status || 'released';
    document.getElementById('game-releaseDate').value = toText(game.releaseDate || game.release || game.year || '');
    const coverValue = toText(game.cover || game.image || '');
    document.getElementById('game-cover').value = coverValue;
    document.getElementById('game-excerpt').value = toText(game.excerpt || game.shortDesc || game.subtitle || '');
    document.getElementById('game-content').value = toText(game.content || game.description || '');
    ...
    document.getElementById('game-modes').value = Array.isArray(game.modes) ? game.modes.join(', ') : toText(game.modes || '');
    document.getElementById('game-difficulty').value = toText(game.difficulty || '');
...
function readForm() {
    ...
    const platforms = platformsStr ? platformsStr.split(',').map(p => p.trim()).filter(p => p) : [];
    const genres = genresStr ? genresStr.split(',').map(g => g.trim()).filter(g => g) : [];
    const modes = modesStr ? modesStr.split(',').map(m => m.trim()).filter(m => m) : [];
...
function generateUniqueSlug(articles, baseSlug, excludeId = null) {
    ...
    while (articles.some(a => a.slug === slug && (!excludeId || a.id !== excludeId))) {
...
function normalizeArticlePayload(payload) {
    ...
    const offerBlock = blocks.find(block => 
        block && (
            block.type === 'offer' || 
...
function validateArticle(payload) {
    ...
    if (!category) {
        ...
        errors['article-category'] = 'La catégorie Tests n\'est plus disponible. Utilise la section Tests dédiée.';
    ...
    if (contentBlocks && Array.isArray(contentBlocks)) {
        contentBlocks.forEach((block, index) => {
            if (block.type === 'text' || block.type === 'heading') {
                const text = toText(block.text);
                if (!text) {
                    errors[`block-${index}`] = `Le bloc ${block.type === 'text' ? 'texte' : 'titre'} ne peut pas être vide`;
                }
            } else if (block.type === 'image') {
                ...
                if (url) {
                    ...
                    const isValidUrl = url.startsWith('http://') || 
                                     ...
                                     url.startsWith('blob:');
                    if (!isValidUrl) {
                        errors[`block-${index}`] = 'L\'URL doit commencer par http(s):// ou être une image en data:image/...';
                    }
                }
                ...
                const url = toText(block.url || block.data?.url || block.link?.url || '');
                const title = toText(block.title || block.data?.title || block.link?.title || '');
                const text = toText(block.text || block.data?.text || block.link?.text || '');
                ...
                if (!vUrl || (!vTitle && !vText)) {
                    errors[`block-${index}`] = "URL et titre/texte de l'offre obligatoires";
                } else {
                ...
                if (!url || !url.trim()) {
                    ...
                    function isTweetUrl(urlStr) {
                        try {
                            ...
                            const hostOk = /(^|\.)x\.com$/.test(u.hostname) || /(^|\.)twitter\.com$/.test(u.hostname);
                    ...
                    if (!isTweetUrl(url)) {
                        errors[`block-${index}`] = 'URL du post X invalide (x.com/.../status/123)';
                    }
                }
            }
        });
...
function fillArticleForm(article) {
    ...
    if (category === 'test') {
        ...
        console.warn(`[admin] Article "${article.title}" a une catégorie obsolète "test", forcée vers "actu".`);
    }
    document.getElementById('article-category').value = category;

    document.getElementById('article-relatedGameId').value = article.relatedGameId || '';
    ...
    if (authorIdField) {
        if (article.authorId) {
            ...
            const authorName = toText(article.author || article.authorName || '').trim();
            if (authorName) {
    ...
    if (authorNameField) {
        ...
        if (article.authorId) {
            ...
            if (author) {
                ...
                authorNameField.value = toText(article.author || article.authorName || '');
            }
        } else {
            authorNameField.value = toText(article.author || article.authorName || '');
        }
    ...
    if (!articleLink || !articleLink.type || articleLink.type === 'none') {
        ...
        if (article.linkType && article.linkType !== 'none') {
            articleLink = {
                type: article.linkType === 'affiliate' ? 'affiliation' : (article.linkType === 'link' ? 'link' : article.linkType),
                url: article.linkUrl || '',
        ...
        else if (article.link && article.link.type && article.link.type !== 'none') {
            articleLink = {
                type: article.link.type === 'affiliate' ? 'affiliation' : (article.link.type === 'link' || article.link.type === 'other' ? 'link' : article.link.type),
                url: article.link.url || '',
                title: article.link.title || '',
                text: article.link.text || '',
                buttonText: article.link.buttonText || article.link.button || article.link.linkCta || article.link.label || '',
        ...
        // Migration depuis cta
            ...
            articleLink = {
                type: article.cta.type === 'affiliate' ? 'affiliation' : (article.cta.type === 'link' || article.cta.type === 'other' ? 'link' : article.cta.type),
                url: article.cta.url || '',
                title: '',
    ...
    document.getElementById('article-featured').checked = article.featured === true;
...
function readArticleForm() {
    ...
    const relatedGameId = document.getElementById('article-relatedGameId').value || '';
    ...
    const publishedAt = publishedAtInput ? toText(publishedAtInput.value).trim() : '';
    ...
    const link = { type: 'none', url: '', title: '', text: '', buttonText: '', discount: '' };
    ...
    if (!slugInputValue && id) {
        ...
        if (existingArticle && existingArticle.slug && existingArticle.slug.trim()) {
...
function syncArticleBlocksFromDOM() {
    ...
    blockElements.forEach((blockEl) => {
        ...
        let block = articleBlocks.find(b => b.blockId === blockId) || { type, blockId: blockId || `block-${Date.now()}-${Math.random().toString(36).substr(2, 9)}` };
        block.type = type;
        
        if (type === 'text') {
            const textarea = blockEl.querySelector('textarea[data-block-field="text"]');
            ...
            const caption = blockEl.querySelector('input[data-block-field="caption"]');
            ...
            const title = (blockEl.querySelector('[data-field="title"]')?.value || '').trim();
            const url = (blockEl.querySelector('[data-field="url"]')?.value || '').trim();
            
            // ÉTAPE 2: Récupérer imageUrl avec fallbacks multiples (robuste)
            const imgInput = blockEl.querySelector('input[data-field="imageUrl"]')
                || blockEl.querySelector('input[placeholder*="URL de l\'image"]')
                || blockEl.querySelector('input[name*="image"]')
                || blockEl.querySelector('input[id*="image"]');
            ...
            const promo = (blockEl.querySelector('[data-field="promo"]')?.value || '').trim();
            const strikePrice = (blockEl.querySelector('[data-field="strikePrice"]')?.value || '').trim();
            const price = (blockEl.querySelector('[data-field="price"]')?.value || '').trim();
            const buttonText = (blockEl.querySelector('[data-field="buttonText"]')?.value || '').trim();
            const note = (blockEl.querySelector('[data-field="note"]')?.value || '').trim();
...
function readArticleBlocks() {
    ...
    return articleBlocks.map(block => {
        ...
        if (blockData.type === 'image' || blockData.type === 'img' || blockData.type === 'media') {
...
function createArticleBlockElement(block, index) {
    ...
    blockEl.dataset.blockIndex = index;
    blockEl.dataset.blockId = block.blockId || `block-${Date.now()}-${index}-${Math.random().toString(36).substr(2, 9)}`;
    blockEl.style.cssText = 'background: rgba(31, 42, 68, 0.3); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--spacing-md);';
    ...
    if (block.type === 'text') {
        ...
        // DEBUG: Log pour vérifier le src réel
        console.log('[YT DEBUG ADMIN] rawUrl=', url, 'id=', videoId, 'embedSrc=', embedSrc, 'isFileProtocol=', isFileProtocol);
        ...
        content = `
            ...
            <div class="youtube-preview" data-block-preview="${index}" style="width: 100%; aspect-ratio: 16/9; border-radius: var(--radius-sm); overflow: hidden; margin-top: var(--spacing-sm); background: rgba(31, 42, 68, 0.5); display: flex; align-items: center; justify-content: center; position: relative;">
                ${videoId ? (isFileProtocol || !embedSrc ? `
    ...
    if (block.type === 'youtube' && block.url) {
        setTimeout(() => {
            ...
            if (ifr) {
                console.log('[YT DEBUG ADMIN] DOM iframe src=', ifr.getAttribute('src'));
...
function renderOfferCard(offerBlock, article = null) {
    ...
    const priceNew = toText(offerBlock.newPrice || offerBlock.priceNew || offerBlock.price || offerBlock.promoPrice) || '';
    ...
    return `<a class="offer-card" href="${escapeHtml(url)}" ${targetAttr}>
        ...
        <div class="offer-card__body">
            ...
            ${(priceOld || priceNew) ? `<div class="offer-card__prices">
...
function moveArticleBlock(index, direction) {
    syncArticleBlocksFromDOM();
    if (direction === 'up' && index > 0) {
        [articleBlocks[index], articleBlocks[index - 1]] = [articleBlocks[index - 1], articleBlocks[index]];
        renderArticleBlocks(articleBlocks);
...
function renderArticlePage() {
    ...
    if (!descriptionText && contentBlocks && Array.isArray(contentBlocks) && contentBlocks.length > 0) {
        ...
        const firstTextBlock = contentBlocks.find(block => block.type === 'text' && block.text);
        if (firstTextBlock) {
    ...
    if (descriptionText) {
        ...
        const truncatedDesc = descriptionText.length > 160 
    ...
    let heroHtml = '';
    if (cover && cover.trim() && (cover.startsWith('http://') || cover.startsWith('https://'))) {
    ...
    if (authorName || publishedAt) {
        ...
        if (authorName && authorName.trim()) {
            ...
            if (authorId && authorId.trim()) {
                ...
                const memberSlug = member ? (normalizeTeamMember(member)?.slug || authorId) : authorId;
                parts.push(`Par <a href="membre.html?member=${escapeHtml(memberSlug)}" style="color: var(--color-accent); text-decoration: none; transition: opacity var(--transition-base);" onmouseover="this.style.opacity='0.8';" onmouseout="this.style.opacity='1';">${escapeHtml(authorName.trim())}</a>`);
            } else {
    ...
    function resolveOfferImage(article) {
        ...
        const linkImageUrl = typeof link.imageUrl === 'string' && link.imageUrl.trim() ? link.imageUrl.trim() : null;
        const linkImage = typeof link.image === 'string' && link.image.trim() ? link.image.trim() : null;
        ...
        const offer = blocks.find(b => {
            const t = (b?.type || '').toLowerCase();
            return t === 'offer' || t === 'affiliation';
        }) || {};

        const candidates = [
            offer.imageUrl, offer.image, offer.img, offer.mediaUrl,
            offer.data?.imageUrl, offer.data?.image, offer.data?.img, offer.data?.mediaUrl,
            offer.link?.imageUrl, offer.link?.image, offer.link?.img, offer.link?.mediaUrl,
        ...
        try {
            ...
            if (raw) {
                ...
                const a = arr.find(x => x.id === article.id || x.slug === article.slug);
                if (a) {
                    const l = a.link || {};
                    if (typeof l.imageUrl === 'string' && l.imageUrl.trim()) return l.imageUrl.trim();
                    if (typeof l.image === 'string' && l.image.trim()) return l.image.trim();
                    const bs = a.contentBlocks || a.blocks || [];
                    const ob = bs.find(b => ((b?.type||'').toLowerCase()==='offer' || (b?.type||'').toLowerCase()==='affiliation')) || {};
                    const c2 = [
                        ob.imageUrl, ob.image, ob.img, ob.mediaUrl,
                        ob.data?.imageUrl, ob.data?.image, ob.link?.imageUrl, ob.link?.image,
                    ].filter(v => typeof v === 'string' && v.trim());
        ...
        blocks.forEach(block => {
            if (block.type === 'text') {
                ...
                if (url) {
                    ...
                    // DEBUG: Log pour vérifier le src réel
                    console.log('[YT DEBUG FRONT] rawUrl=', url, 'id=', videoId, 'embedSrc=', embedSrc, 'isFileProtocol=', isFileProtocol);
...
function renderArticleContent() {
    ...
    if (blocks.length > 0) {
        ...
        setTimeout(() => {
            ...
            if (ifr) {
                console.log('[YT DEBUG FRONT] DOM iframe src=', ifr.getAttribute('src'));
            }
        }, 0);
    } else {
...
function closeArticleModal() {
    ...
    if (modal) {
        ...
        const previewPlaceholder = document.getElementById('articleCoverPreviewPlaceholder');
...
function updateArticleCoverPreview(url, { silent = true } = {}) {
    ...
    if (!urlPattern.test(urlTrimmed)) {
        ...
        if (coverError) coverError.textContent = 'L\'URL doit commencer par http:// ou https://';
        return;
    }
    ...
    img.onerror = function() {
        ...
        try {
            ...
            if (coverInput) coverInput.classList.add('invalid', 'is-invalid');
            if (coverError) coverError.textContent = 'Image introuvable ou URL invalide';
        } catch (e) {
...
function createArticle(payload) {
    ...
    const newArticle = {
        ...
        link: payload.link || { type: 'none', url: '', title: '', text: '', buttonText: '', discount: '' } // ADDED: Objet link standardisé
    };
    
    // ADDED: Ajouter contentBlocks si présent
    if (payload.contentBlocks && Array.isArray(payload.contentBlocks) && payload.contentBlocks.length > 0) {
    ...
    console.log('[admin] saved article (create)', newArticle.id, 'contentBlocks:', newArticle.contentBlocks?.length, 'blocks:', newArticle.blocks?.length);
    console.debug("[admin] saved contentBlocks sample (create)", newArticle.contentBlocks?.filter(b => b.type === "image").slice(0, 2));
    renderAdminArticlesList();
    closeArticleModal();
}
...
function updateArticle(id, payload) {
    ...
    Object.assign(article, {
        ...
        publishedAt: (payload.publishedAt || '').trim(),
        link: payload.link || { type: 'none', url: '', title: '', text: '', button: '', discount: '' }, // ADDED: Objet link standardisé
        updatedAt: now
    });
    ...
    // ADDED: Debug preuve après save (admin uniquement)
    console.log('[admin] saved article (update)', article.id, 'contentBlocks:', article.contentBlocks?.length, 'blocks:', article.blocks?.length);
    console.debug("[admin] saved contentBlocks sample (update)", article.contentBlocks?.filter(b => b.type === "image").slice(0, 2));
    renderAdminArticlesList();
    closeArticleModal();
}
...
function renderAdminArticlesList() {
    ...
    list.innerHTML = articles.map(article => {
        const game = article.relatedGameId ? games.find(g => g.id === article.relatedGameId) : null;
...
function renderAdminBugsList() {
    ...
    list.innerHTML = bugs.map(bug => {
        ...
        const clearedDate = bug.clearedAt ? new Date(bug.clearedAt).toLocaleDateString('fr-FR') : '';
...
function renderAdminContactRequestsList() {
    ...
    list.innerHTML = requests.map(request => {
        ...
        const createdTime = new Date(request.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
...
function renderAdminUsersLogsList() {
    ...
    list.innerHTML = logs.map(log => {
        ...
        const createdTime = new Date(log.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
...
function updateGame(id, payload) {
    ...
    games[index] = normalizeGame({
        ...
        shortDesc: payload.excerpt || payload.shortDesc || existingGame.shortDesc || existingGame.excerpt || existingGame.subtitle || '',
        content: payload.content || existingGame.content || existingGame.description || '',
        ...
        difficulty: payload.difficulty !== undefined ? payload.difficulty : existingGame.difficulty,
        duration: payload.duration !== undefined ? payload.duration : existingGame.duration,
        isFeatured: payload.isFeatured !== undefined ? payload.isFeatured : (existingGame.isFeatured || false)
    });
...
function initPublicPages() {
    ...
    console.log('[DEBUG] initPublicPages called - pathname:', window.location.pathname);
    ...
    if (!sponsorFocusHandlerAdded) {
        ...
        const preventSponsorFocus = (e) => {
            ...
            const focusableElements = sponsorElement.querySelectorAll('a, button, input, [tabindex]');
    ...
    if (window.location.pathname.includes('actu.html')) {
        renderArticlesMosaic({
            ...
            filterFn: (article) => ['actu', 'actualite', 'actualites', 'actualité', 'actualités'].includes(normalizeCategory(article.category || '')),
            title: 'Actu',
        ...
        renderArticlesMosaic({
            containerId: 'articlesGrid',
            filterFn: (article) => ['guide', 'guides'].includes(normalizeCategory(article.category || '')),
            title: 'Guides',
        ...
        console.log('[BONSPLANS] hero hidden:', !heroEl, 'filters hidden:', !filtersEl);
...
function renderCategoryArticles(category) {
    ...
    let container = document.getElementById('articles-list') || 
    ...
    let articlesContainer = document.getElementById('articles-list') || document.getElementById('articles-grid');
    if (!articlesContainer) {
        ...
        if (welcomeSection) {
            ...
            articlesContainer.style.cssText = 'display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: var(--spacing-lg); margin-top: var(--spacing-2xl);';
 ...
 */
function renderLatestArticlesSidebar({ containerId = 'latestArticlesSidebar', limit = 10 } = {}) {
    ...
    try {
        const articles = (typeof getArticlesData === "function") ? getArticlesData() : (window.ARTICLES || []);

        if (!articles || articles.length === 0) {
            container.innerHTML = '<p style="color: var(--color-text-secondary); font-size: 0.875em; text-align: center; padding: var(--spacing-md);">Aucun article disponible</p>';
            return;
        }
        ...
        container.innerHTML = latestArticles.map(article => {
            const href = getArticleUrl ? getArticleUrl(article) : `article.html?id=${article.id}`;
            const title = article.title || 'Sans titre';
            const excerpt = article.excerpt || '';
        ...
        container.innerHTML = '<p style="color: var(--color-text-secondary); font-size: 0.875em; text-align: center; padding: var(--spacing-md);">Aucun article pour le moment.</p>';
    }
}
...
function renderHomeArticlesEditorial(container) {
    ...
    try {
        ...
        if (!articles || articles.length === 0) {
            container.innerHTML = '<p style="color: var(--color-text-secondary);">Aucun article disponible.</p>';
            return;
        }
        ...
        const hero = featuredList.length > 0 ? featuredList[0] : (sorted.length > 0 ? sorted[0] : null);
        ...
        const derniers = sorted.filter(a => a.id !== heroId && !aLaUneIds.has(a.id));
        ...
        if (hero) {
            ...
            let heroImageHtml = '';
            if (hero.cover && hero.cover.trim() && (hero.cover.startsWith('http://') || hero.cover.startsWith('https://'))) {
        ...
        if (derniers.length > 0) {
            html += `
                <div>
                    ...
                    <div class="articles-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: var(--spacing-lg);">
                        ${derniers.slice(0, 6).map(article => renderArticleCard(article)).join('')}
                    </div>
                </div>
            `;
        }
        ...
        // ADDED: Fallback silencieux (zéro console error)
        container.innerHTML = '<p style="color: var(--color-text-secondary);">Erreur de chargement des articles.</p>';
    }
}
...
function renderHomeArticlesSidebar(grid) {
    ...
    try {
        ...
        // ADDED: Générer les cards HTML
        grid.innerHTML = topArticles.map(article => renderArticleCard(article)).join('');
...
function isArticleLinkedToGame(article, game) {
    ...
    if (article.linkedGameId && String(article.linkedGameId) === String(game.id)) {
        return true;
    }
    
    // ADDED: Priorité 2: relatedGameId === game.id (champ utilisé dans data.js)
    if (article.relatedGameId && String(article.relatedGameId) === String(game.id)) {
        return true;
    }
...
function renderLinkedArticles(game) {
    ...
    const linkedArticles = articles.filter(article => isArticleLinkedToGame(article, game));
...
function renderArticlesMosaic({ containerId, filterFn, title, subtitle }) {
    ...
    try {
        ...
        const allArticles = getArticlesData();
        const filteredArticles = filterFn ? allArticles.filter(filterFn) : allArticles;
        ...
        function renderArticleItem(article) {
            const href = getArticleUrl ? getArticleUrl(article) : `article.html?id=${article.id}`;
            const category = article.category || '';
            const imageHtml = article.cover && article.cover.trim() && article.cover.startsWith('http')
        ...
        container.innerHTML = '<p style="color: var(--color-error); text-align: center; padding: var(--spacing-xl);">Erreur de chargement des articles</p>';
    }
}
...
function renderGamePage() {
    ...
    const hasValidImage = gameImage && gameImage !== 'placeholder' && gameImage.trim() && (gameImage.startsWith('http://') || gameImage.startsWith('https://'));
    ...
    if (game.releaseDate && game.releaseDate.trim()) {
        infoItems.push(`<div class="info-item"><span class="info-label">Sortie</span><span class="info-value">${escapeHtml(game.releaseDate)}</span></div>`);
    }
    if (game.studio && game.studio.trim()) {
        infoItems.push(`<div class="info-item"><span class="info-label">Studio</span><span class="info-value">${escapeHtml(game.studio)}</span></div>`);
    }
    if (game.duration && game.duration.trim()) {
        infoItems.push(`<div class="info-item"><span class="info-label">Durée</span><span class="info-value">${escapeHtml(game.duration)}</span></div>`);
    }
    if (game.difficulty && game.difficulty.trim()) {
        infoItems.push(`<div class="info-item"><span class="info-label">Difficulté</span><span class="info-value">${escapeHtml(game.difficulty)}</span></div>`);
    }
    if (game.modes && Array.isArray(game.modes) && game.modes.length > 0) {
        infoItems.push(`<div class="info-item"><span class="info-label">Mode</span><span class="info-value">${escapeHtml(game.modes.join(' • '))}</span></div>`);
    }
...
function renderMemberAvatar(member, size) {
    ...
    if (member.avatarType === 'image' && member.avatarUrl && member.avatarUrl.trim()) {
    ...
    // Fallback initiales
    const initials = (member.initials || '').trim() || (member.name ? member.name.split(' ').map(p => p[0]).join('').slice(0, 2).toUpperCase() : '?');
    return `
...
function renderTeamPage() {
    ...
    try {
        ...
        const teamData = getTeamData();
        const allMembers = (teamData && Array.isArray(teamData.members)) ? teamData.members : [];
        ...
        if (teamHero && teamData) {
            ...
            const memberText = memberCount === 1 ? '1 membre' : `${memberCount} membres`;
            if (subtitleEl) subtitleEl.textContent = `${teamData.aboutText || 'Les admins et rédacteurs du site'} • ${memberText}`;
        }
        
        if (members.length === 0) {
            teamGrid.innerHTML = '<p style="text-align: center; color: var(--color-text-secondary); padding: var(--spacing-3xl);">Aucun membre pour le moment.</p>';
            return;
        }
        ...
        const articleCountText = articleCount === 0 ? 'Aucun article' : articleCount === 1 ? '1 article' : `${articleCount} articles`;
        ...
        if (teamGrid) {
            teamGrid.innerHTML = '<p style="text-align: center; color: var(--color-text-secondary); padding: var(--spacing-3xl);">Erreur lors du chargement de l\'équipe.</p>';
        }
    }
}
...
function renderMemberProfile(memberId, container, hero) {
    ...
    try {
        ...
        if (container) {
            container.innerHTML = '<p style="text-align: center; color: var(--color-text-secondary); padding: var(--spacing-3xl);">Erreur lors du chargement du profil.</p>';
        }
    }
}
...
function renderAdminTeamList() {
    ...
    if (members.length === 0) {
        list.innerHTML = '<div class="empty-state">Aucun membre pour le moment.</div>';
        return;
    }
...
function editTeamMember(memberId) {
    ...
    if (!modal || !form || !idInput || !nameInput || !roleSelect || !avatarTypeSelect) {
    ...
    if (avatarMediaIdInput) avatarMediaIdInput.value = normalized.avatarMediaId || '';
    if (initialsInput) initialsInput.value = normalized.initials || normalized.avatarText || '';
    ...
    if (normalized.avatarType === 'image' && normalized.avatarUrl) {
        ...
        setTimeout(() => {
            if (typeof window.updateAvatarCropPreview === 'function') {
                window.updateAvatarCropPreview(crop.x || 0, crop.y || 0, crop.scale || 1);
            }
        }, 100);
    }
...
function toggleAvatarTypeFields(avatarType) {
    ...
    const initialsGroup = document.getElementById('team-member-avatar-initials-group');
    
    if (urlGroup) urlGroup.style.display = avatarType === 'url' ? 'block' : 'none';
    if (mediaGroup) mediaGroup.style.display = avatarType === 'media' ? 'block' : 'none';
    if (imageGroup) imageGroup.style.display = avatarType === 'image' ? 'block' : 'none';
    if (initialsGroup) initialsGroup.style.display = avatarType === 'initials' ? 'block' : 'none';
...
function initAvatarCropper() {
    ...
    const pickAvatarMediaBtn = document.getElementById('team-member-pick-avatar-media');
...
function handleTeamMemberFormSubmit(e) {
    ...
    const avatarTypeSelect = document.getElementById('team-member-avatar-type');
    const avatarUrlInput = document.getElementById('team-member-avatar-url');
    const avatarImageUrlInput = document.getElementById('team-member-avatar-image-url'); // ADDED: Fix crash - déclarer la variable
    const avatarMediaIdInput = document.getElementById('team-member-avatar-media-id');
    const initialsInput = document.getElementById('team-member-initials');
    const isActiveInput = document.getElementById('team-member-is-active');
    const youtubeInput = document.getElementById('team-member-youtube');
    ...
    if (!idInput || !nameInput || !roleSelect || !orderInput || !avatarTypeSelect) {
    ...
    if (!role || (role !== 'directeur' && role !== 'createur' && role !== 'redacteur')) {
    ...
    if (avatarType === 'image') {
        ...
        avatarUrl = avatarImageUrlInput ? toText(avatarImageUrlInput.value).trim() : '';
        if (!avatarUrl || (!avatarUrl.startsWith('http://') && !avatarUrl.startsWith('https://') && !avatarUrl.startsWith('data:image/'))) {
            alert('L\'URL de l\'avatar image doit commencer par http://, https:// ou data:image/');
            return;
        }
        ...
        avatarMediaId = avatarMediaIdInput ? toText(avatarMediaIdInput.value).trim() : '';
        if (!avatarMediaId) {
        ...
        initials = initialsInput ? toText(initialsInput.value).trim().substring(0, 3).toUpperCase() : '';
        if (!initials) {
    ...
    const finalMemberId = isCreating ? `mem_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : memberId;
...
function initAdminPromoPage() {
    ...
    if (sponsorImageInput) {
        sponsorImageInput.addEventListener('input', () => {
            updateImagePreview('sponsor-imageUrl', 'sponsorImagePreviewImg', 'sponsorImagePreviewPlaceholder');
        });
    }

    // ADDED: Bouton "Choisir depuis Médias" pour sponsor
    const btnChooseMediaSponsor = document.getElementById('btn-choose-media-sponsor');
...
function renderAdminPromosList() {
    ...
    if (promos.length === 0) {
        list.innerHTML = '<div class="empty-state" style="color: var(--color-text-secondary); text-align: center; padding: var(--spacing-xl);">Aucune promo pour le moment</div>';
        return;
    }
    ...
    list.innerHTML = promos.map(promo => {
        ...
        const priceNew = toText(promo.price || promo.newPrice || promo.priceNew) || '';
        ...
        if (priceOld || priceNew) {
            const oldPriceHtml = priceOld ? `<span style="text-decoration: line-through; margin-right: var(--spacing-xs);">${escapeHtml(priceOld)}</span>` : '';
            const newPriceHtml = priceNew ? `<span style="color: var(--color-accent); font-weight: var(--font-weight-semibold);">${escapeHtml(priceNew)}</span>` : '';
...
function updateImagePreview(inputId, previewImgId, placeholderId) {
    ...
    const previewPlaceholder = placeholderId ? document.getElementById(placeholderId) : null;
    ...
    try {
        ...
        if (!urlObj.hostname || urlObj.hostname === 'gaming-cdn.co' || (urlObj.hostname.includes('localhost') && !urlObj.hostname.includes('127.0.0.1'))) {
            previewImg.style.display = 'none';
            if (previewPlaceholder) {
                previewPlaceholder.style.display = 'block';
                previewPlaceholder.textContent = 'Aperçu indisponible';
...
function renderAdminSponsorsList() {
    ...
    const sponsors = (typeof getSponsorsData === "function")
        ? getSponsorsData()
    ...
    if (sponsorsData.items.length === 0) {
        list.innerHTML = '<div class="empty-state" style="color: var(--color-text-secondary); text-align: center; padding: var(--spacing-xl);">Aucun sponsor pour le moment</div>';
        return;
    }

    list.innerHTML = sponsorsData.items.map(sponsor => {
        // Vérifier si ce sponsor est actif selon ses propres propriétés
        const isActive = sponsor.active === true;
        const statusClass = isActive ? 'sponsor-item__status--active' : 'sponsor-item__status--inactive';
...
function openSponsorModal(mode, sponsor = null) {
    ...
    if (mode === 'edit' && sponsor) {
        ...
        document.getElementById('sponsor-ctaText').value = sponsor.ctaText || 'Découvrir';
        document.getElementById('sponsor-imageUrl').value = sponsor.imageUrl || '';
        document.getElementById('sponsor-openNewTab').checked = sponsor.openNewTab || false;
        document.getElementById('sponsor-active').checked = sponsor.active || false;
        document.getElementById('sponsor-startAt').value = sponsor.startAt ? sponsor.startAt.split('T')[0] : '';
        document.getElementById('sponsor-endAt').value = sponsor.endAt ? sponsor.endAt.split('T')[0] : '';
...
function editSponsor(sponsorId) {
    const sponsorsData = (typeof getSponsorsData === "function")
        ? getSponsorsData()
        : (window.SPONSORS || window.SPONSORS_DATA || { items: [] });
...
function saveSponsor(formData) {
    try {
        ...
        const existingIndex = sponsorsData.items.findIndex(s => s.id === sponsor.id);
        if (existingIndex >= 0) {
...
function openPromoModal(mode, promo = null) {
    ...
    if (mode === 'edit' && promo) {
        ...
        document.getElementById('promo-imageUrl').value = toText(promo.imageUrl || promo.image) || '';
        document.getElementById('promo-promo').value = toText(promo.promo || promo.promoText) || '';
        document.getElementById('promo-oldPrice').value = toText(promo.oldPrice || promo.priceOld) || '';
        document.getElementById('promo-price').value = toText(promo.price || promo.newPrice || promo.priceNew) || '';
        document.getElementById('promo-buttonText').value = toText(promo.buttonText) || 'Voir l\'offre';
...
function updatePromoImagePreview(url) {
    ...
    const previewPlaceholder = document.getElementById('promoImagePreviewPlaceholder');
...
function handlePromoFormSubmit(e) {
    ...
    const buttonText = toText(form.querySelector('#promo-buttonText').value) || 'Voir l\'offre';
    ...
    const promoData = {
        ...
        buttonText,
        newTab,
        createdAt: isEditMode ? (promos.find(p => p.id === id)?.createdAt || new Date().toISOString()) : new Date().toISOString()
    };
...
function renderHomePromos() {
    ...
    try {
        ...
        if (promos.length === 0) {
            grid.innerHTML = '<div style="color: var(--color-text-secondary); text-align: center; padding: var(--spacing-md);">Aucune promo pour le moment</div>';
            return;
        }
        ...
        grid.innerHTML = limitedPromos.map(promo => renderOfferCard(promo)).join('');
        ...
        grid.innerHTML = '<div style="color: var(--color-text-secondary); text-align: center; padding: var(--spacing-md);">Erreur de chargement</div>';
    }
}
...
function mountPromosWidget() {
    ...
    try {
        ...
        grid.innerHTML = sortedPromos.map(promo => renderOfferCard(promo)).join('');
    } catch (e) {
...
function renderSponsorsCarousel(containerId = "sponsorSlot") {
    ...
    try {
        ...
        if (sponsorsData.items && sponsorsData.items.length > 0) {
            activeSponsors = sponsorsData.items.filter(sponsor => sponsor.active === true);
        }
        ...
        activeSponsors.forEach((sponsor, index) => {
            const targetAttr = sponsor.openNewTab ? 'target="_blank" rel="noopener"' : '';
        ...
        container.innerHTML = '<div style="text-align: center; color: var(--color-text-secondary); padding: var(--spacing-md); font-size: 0.875em;">Espace publicitaire</div>';
    }
}
...
function renderPartnershipBlock() {
    ...
    try {
        ...
        activeItems.forEach((item, index) => {
            const targetAttr = item.openNewTab ? 'target="_blank" rel="noopener"' : '';
...
function handleLoginSubmit(e) {
    ...
    const form = e.target;
    const identifier = toText(form.querySelector('#login-identifier').value).trim();
    ...
    window.fb.signInWithEmailAndPassword(window.fb.auth, identifier, password)
        .then((userCredential) => {
            // Connexion réussie
...
function handleSignupSubmit(e) {
    ...
    const passwordConfirm = toText(form.querySelector('#signup-password-confirm').value);
...
if (document.getElementById('loginForm')) {
    ...
    if (loginPassword) {
        loginPassword.addEventListener('input', () => clearFormError('login-password'));
    }
}
...
if (document.getElementById('signupForm')) {
    ...
    if (signupPseudo) {
        signupPseudo.addEventListener('input', () => clearFormError('signup-pseudo'));
    }
    if (signupEmail) {
        signupEmail.addEventListener('input', () => clearFormError('signup-email'));
    }
    if (signupPassword) {
        signupPassword.addEventListener('input', () => clearFormError('signup-password'));
    }
    if (signupPasswordConfirm) {
        signupPasswordConfirm.addEventListener('input', () => clearFormError('signup-password-confirm'));
    }
}
